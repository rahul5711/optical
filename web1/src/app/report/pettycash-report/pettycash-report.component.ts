...
export class PettycashReportComponent implements OnInit {
 ...
 constructor(
  ...
  data1: any = {
    FromDate: moment().startOf('day').format('YYYY-MM-DD'), ToDate: moment().format('YYYY-MM-DD'), ShopID: 0, CashType: 0,
  };
  ...
  OpeningClosing() {
    ...
    if (this.data1.FromDate !== '' && this.data1.FromDate !== null) {
      ...
      Parem = Parem + ' and DATE_FORMAT(pettycashreport.Date, "%Y-%m-%d") between ' + `'${FromDate}'`;
    }
    ...
    if (this.data1.ShopID != 0) {
      Parem = Parem + ' and pettycashreport.ShopID IN ' + `(${this.data1.ShopID})`;
    }

    if (this.data1.CashType != 0) {
      Parem = Parem + '  and pettycashreport.RegisterType = ' +  `'${this.data1.CashType}'`;
    }

    const subs: Subscription = this.pettyCash.pettyCashOpeningClosingReport(Parem).subscribe({
      next: (res: any) => {
        if (res.success) {
          this.as.successToast(res.message)
  ...
  print1() {
    ...
    let printContent: any = document.getElementById('pettyOC-content');
    let printWindow: any = window.open('pp', '_blank');
    printWindow.document.write(`
    <html>
      <head>
  ...
  exportAsXLSX1(): void {
    let element = document.getElementById('pettyOCExcel');
    const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(element);
    delete ws['A2'];
    // Initialize column widths array
    const colWidths: number[] = [];
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'pettyCashOpeningClosing_Report.xlsx');
  }
