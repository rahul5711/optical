        ...
        switch (TableName) {
            case "Product":
                if (Body.ID === null || Body.ID === undefined) {
                    qry = `insert into product (Name,  CompanyID, HSNCode,GSTPercentage,GSTType, Status, CreatedBy , CreatedOn ) values ('${Body.Name}', ${CompanyID}, '${Body.HSNCode}', ${Body.GSTPercentage}, '${Body.GSTType}', 1 , ${LoggedOnUser.ID}, now())`;
                } else {
                    qry = `update product set Name = '${Body.Name}' , CompanyID = ${CompanyID} , HSNCode = '${Body.HSNCode}', GSTPercentage = ${Body.GSTPercentage}, GSTType = '${Body.GSTType}' ,Status = 1 , UpdatedOn = now(), UpdatedBy = ${LoggedOnUser.ID} where ID = ${Body.ID}`;
                }
                break;
            case "getProduct":
