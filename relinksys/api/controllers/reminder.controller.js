...
module.exports = {
    ...
    getFeedBackReminder: async (req, res, next) => {
        ...
        try {
            ...
            let qry = `select DISTINCT(billmaster.ID),customer.Title, customer.Name, customer.MobileNo1, customer.Email, billmaster.ShopID, COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'), NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'), billmaster.OrderDate) AS BillDate from billdetail left join billmaster on billmaster.ID = billdetail.BillID left join customer on customer.ID = billmaster.CustomerID where billdetail.CompanyID = ${CompanyID} and billdetail.ProductTypeName IN ('FRAME', 'LENS', 'CONTACT LENS', 'SUNGLASS') ${shopId} AND DATE(COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'),NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'),billmaster.OrderDate)) = DATE_SUB('${date}', INTERVAL ${feedbackDays} DAY)`
    ...
    getServiceMessageReminder: async (req, res, next) => {
        ...
        try {
            ...
            let qry = `select DISTINCT(billmaster.ID), customer.Title, customer.Name, customer.MobileNo1, COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'), NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'), billmaster.OrderDate) AS BillDate, customer.Email, billmaster.ShopID from billdetail left join billmaster on billmaster.ID = billdetail.BillID left join customer on customer.ID = billmaster.CustomerID where billdetail.CompanyID = ${CompanyID} and billdetail.ProductTypeName IN ('FRAME', 'LENS', 'CONTACT LENS', 'SUNGLASS')  ${shopId} AND DATE(COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'),NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'),billmaster.OrderDate)) = DATE_SUB('${date}', INTERVAL ${serviceDays} DAY)`
...
async function getFeedBackReminder(CompanyID, shopid, db) {
    ...
    try {
        ...
        let qry = `select DISTINCT(billmaster.ID),customer.Title, customer.Name, customer.MobileNo1, COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'), NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'), billmaster.OrderDate) AS BillDate from billdetail left join billmaster on billmaster.ID = billdetail.BillID left join customer on customer.ID = billmaster.CustomerID where billdetail.CompanyID = ${CompanyID} and billdetail.ProductTypeName IN ('FRAME', 'LENS', 'CONTACT LENS', 'SUNGLASS') ${shopId} AND DATE(COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'),NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'),billmaster.OrderDate)) = DATE_SUB('${date}', INTERVAL ${feedbackDays} DAY)`
        ...
        if (company.length) {
            for (let data of company) {
                ...
                if (fetchCompanySetting[0].IsComfortFeedBackReminder === true || fetchCompanySetting[0].IsComfortFeedBackReminder === "true") {
                    ...
                    const [qry] = await connection.query(`select DISTINCT(billmaster.ID),customer.Title, customer.Name, customer.MobileNo1, customer.Email, billmaster.BillDate, billmaster.ShopID,'Customer_Comfort Feedback' as Type, 'FeedBack Reminder' as MailSubject  from billdetail left join billmaster on billmaster.ID = billdetail.BillID left join customer on customer.ID = billmaster.CustomerID where billdetail.CompanyID = ${CompanyID} and customer.Email != '' and billdetail.ProductTypeName IN ('FRAME', 'LENS', 'CONTACT LENS', 'SUNGLASS') AND DATE(COALESCE(NULLIF(billmaster.BillDate,'0000-00-00'),NULLIF(billmaster.BillDate,'0000-00-00 00:00:00'),billmaster.OrderDate)) = DATE_SUB('${service_date}', INTERVAL ${feedbackDays} DAY)`)
