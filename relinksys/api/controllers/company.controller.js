  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; margin: auto; background-color: #ffffff; padding: 20px; border: 1px solid #ddd;">
    ...
    updatePlan: async (req, res, next) => {
        ...
        try {
            ...
            const [updateCompany] = await DB.query(`update company set PrimeMembership = '${Body.PrimeMembership}',PhotoClick = '${Body.PhotoClick}',CustomerCategory = '${Body.CustomerCategory}',EmployeeCommission = '${Body.EmployeeCommission}',LoginHistory = '${Body.LoginHistory}',DiscountSetting = '${Body.DiscountSetting}',Quotation = '${Body.Quotation}',ProductTransfer = '${Body.ProductTransfer}',BulkTransfer = '${Body.BulkTransfer}',PettyCash = '${Body.PettyCash}',LocationTracker = '${Body.LocationTracker}',StockCheck = '${Body.StockCheck}',RecycleBin = '${Body.RecycleBin}',AllExcelImport = '${Body.AllExcelImport}',OldBill = '${Body.OldBill}', InsuranceModule = '${Body.InsuranceModule ? Body.InsuranceModule : false}', OrderRequest = '${Body.OrderRequest ? Body.OrderRequest : false}', LensGrid = '${Body.LensGrid ? Body.LensGrid : false}' where ID = ${Body.ID}`)
