...
module.exports = {
    create: async (req, res, next) => {
        ...
        try {
            ...
            if (!Body?.DBkey || Body.DBkey === undefined || Body.DBkey === null || Body.DBkey.trim() === "") {
                return res.send({ message: "Invalid Query Data" })
            }


            const [doesExist] = await DB.query(`select * from company where Email = '${Body.Email}' and Status = 1`)
            if (doesExist.length) return res.send({ message: `Company Already exist from this Email ${Body.Email}` })


            const [doesExistUser] = await DB.query(`select * from user where Email = '${Body.User.Email}' and Status = 1`)
            if (doesExistUser.length) return res.send({ message: `User Already exist from this Email ${Body.User.Email}` })

            const [doesExistLoginName] = await DB.query(`select * from user where LoginName = '${Body.User.LoginName}'`)
            ...
            const [saveCompany] = await DB.query(`insert into company (Name,  MobileNo1,  MobileNo2,  PhoneNo,  Address, Country, State, City,  Email,  Website,  GSTNo,  CINNo,  LogoURL,  Remark, SRemark, CAmount,  Plan, Version, NoOfShops,  EffectiveDate,  CancellationDate, EmailMsg, WhatsappMsg, IsPaidWhatsappMsg, WholeSale,RetailPrice,  Status, CompanyStatus, CreatedBy , CreatedOn, Code, DBkey,PrimeMembership,PhotoClick,CustomerCategory,EmployeeCommission,LoginHistory,DiscountSetting,Quotation,ProductTransfer,BulkTransfer,PettyCash,LocationTracker,StockCheck,RecycleBin,AllExcelImport,OldBill, SubDomainName ) values ('${Body.CompanyName}', '${Body.MobileNo1}', '${Body.MobileNo2}', '${Body.PhoneNo}', '${Body.Address}', '${Body.Country}', '${Body.State}', '${Body.City}', '${Body.Email}','${Body.Website}','${Body.GSTNo}','${Body.CINNo}','${Body.LogoURL}','${Body.Remark}','${Body.SRemark}','${Body.CAmount}',${Body.Plan},'${Body.Version}',${Body.NoOfShops}, '${Body.EffectiveDate}', '${Body.CancellationDate}', '${Body.EmailMsg}',  '${Body.WhatsappMsg}', '${Body.IsPaidWhatsappMsg ? Body.IsPaidWhatsappMsg : 'false'}','${Body.WholeSale}', '${Body.RetailPrice}', 1, ${Body.CompanyStatus} , ${LoggedOnUser}, now(), '${Body.Code ? Body.Code : 91}', '${Body.DBkey}','${Body.PrimeMembership}','${Body.PhotoClick}','${Body.CustomerCategory}','${Body.EmployeeCommission}','${Body.LoginHistory}','${Body.DiscountSetting}','${Body.Quotation}','${Body.ProductTransfer}','${Body.BulkTransfer}','${Body.PettyCash}','${Body.LocationTracker}','${Body.StockCheck}','${Body.RecycleBin}','${Body.AllExcelImport}','${Body.OldBill}', '${Body.SubDomainName ? Body.SubDomainName : ''}' )`)

            const [saveCompany2] = await connection.query(`insert into company (ID,Name,  MobileNo1,  MobileNo2,  PhoneNo,  Address, Country, State, City,  Email,  Website,  GSTNo,  CINNo,  LogoURL,  Remark, SRemark, CAmount,  Plan, Version, NoOfShops,  EffectiveDate,  CancellationDate, EmailMsg, WhatsappMsg, IsPaidWhatsappMsg, WholeSale,RetailPrice,  Status, CreatedBy , CreatedOn, Code, PrimeMembership,PhotoClick,CustomerCategory,EmployeeCommission,LoginHistory,DiscountSetting,Quotation,ProductTransfer,BulkTransfer,PettyCash,LocationTracker,StockCheck,RecycleBin,AllExcelImport,OldBill, SubDomainName ) values (${saveCompany.insertId},'${Body.CompanyName}', '${Body.MobileNo1}', '${Body.MobileNo2}', '${Body.PhoneNo}', '${Body.Address}', '${Body.Country}', '${Body.State}', '${Body.City}', '${Body.Email}','${Body.Website}','${Body.GSTNo}','${Body.CINNo}','${Body.LogoURL}','${Body.Remark}','${Body.SRemark}','${Body.CAmount}',${Body.Plan},'${Body.Version}',${Body.NoOfShops}, '${Body.EffectiveDate}', '${Body.CancellationDate}', '${Body.EmailMsg}',  '${Body.WhatsappMsg}','${Body.IsPaidWhatsappMsg ? Body.IsPaidWhatsappMsg : 'false'}','${Body.WholeSale}', '${Body.RetailPrice}', 1 , ${LoggedOnUser}, now(), '${Body.Code ? Body.Code : 91}', '${Body.PrimeMembership}','${Body.PhotoClick}','${Body.CustomerCategory}','${Body.EmployeeCommission}','${Body.LoginHistory}','${Body.DiscountSetting}','${Body.Quotation}','${Body.ProductTransfer}','${Body.BulkTransfer}','${Body.PettyCash}','${Body.LocationTracker}','${Body.StockCheck}','${Body.RecycleBin}','${Body.AllExcelImport}','${Body.OldBill}', '${Body.SubDomainName ? Body.SubDomainName : ''}' )`)
            ...
            const [saveUser] = await DB.query(`insert into user(CompanyID,Name,UserGroup,DOB,Anniversary,MobileNo1,MobileNo2,PhoneNo,Email,Address,Branch,PhotoURL,Document,LoginName,Password,Status,CreatedBy,UpdatedBy,CreatedOn,UpdatedOn,CommissionType,CommissionMode,CommissionValue,CommissionValueNB) values(${saveCompany.insertId},'${Body.User.Name}','CompanyAdmin','${Body.User.DOB}','${Body.User.Anniversary}','${Body.User.MobileNo1}','${Body.User.MobileNo2}','${Body.User.PhoneNo}','${Body.User.Email}','${Body.User.Address}','${Body.User.Branch}','${Body.User.PhotoURL}','${Body.User.Document}','${Body.User.LoginName}','${pass}',1,0,0,now(),now(),${Body.User.CommissionType},${Body.User.CommissionMode},${Body.User.CommissionValue},${Body.User.CommissionValueNB})`)
            ...
            if (Body.dataAssign === true) {
                ...
                const [product] = await DB.query(`SELECT ${saveCompany.insertId} as CompanyID ,product.Name, product.HSNCode, product.GSTPercentage,product.GSTType, product.Status, 0 AS CreatedBy, NOW() AS CreatedOn FROM product WHERE Status = 1 AND CompanyID = 0`)
                let result = []
                if (product) {
                ...
                let [suport_master_table_data] = await DB.query(`select * from supportmaster where Status =1 and CompanyID = 0`)
            ...
            if (!fetchCategory.length) {
                ...
                let suport_master_table = []
                let [suport_master_table_data] = await DB.query(`select * from supportmaster where Status =1 and CompanyID = 0 and TableName = 'CustomerCategory'`)
            ...
            const [savesupplier] = await connection.query(`insert into supplier (Sno,Name, CompanyID,  MobileNo1, MobileNo2 , PhoneNo, Address,GSTNo, Email,Website ,CINNo,Fax,PhotoURL,ContactPerson,Remark,GSTType,DOB,Anniversary, Status,CreatedBy,CreatedOn) values ('1','PreOrder Supplier', ${saveCompany.insertId}, 'xxxxxxxxxx', 'xxxxxxxxxx', 'xxxxxxxxxx','Pune','','','','','','','','','','0000-00-00','0000-00-00',1,${LoggedOnUser}, now())`)
            ...
            const [saveinvoice] = await connection.query(`insert into invoice(CompanyID, ShopID, Retail, WholeSale, Service, invoice.Order, Ecommerce, CreatedOn)values(${saveCompany.insertId},0,1,1,1,1,1,now())`);
            ...
            const [Company] = await DB.query(`select * from company where ID = ${saveCompany.insertId}`)
            const [User] = await DB.query(`select * from user where ID = ${saveUser.insertId}`)
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; margin: auto; background-color: #ffffff; padding: 20px; border: 1px solid #ddd;">
    ...
    updatePassword: async (req, res, next) => {
        ...
        try {
            ...
            const [doesExist] = await DB.query(`select ID, CompanyID from user where ID = ${Body.ID} and Status = 1`)
    ...
    update: async (req, res, next) => {
        ...
        try {
            ...
            const [doesExist] = await DB.query(`select ID from company where Email = '${Body.Email}' and Status = 1 and ID != ${Body.ID}`)
            if (doesExist.length) return res.send({ message: `Company Already exist from this Email ${Body.Email}` })


            const [doesExistUser] = await DB.query(`select ID from user where Email = '${Body.User.Email}' and Status = 1 and ID != ${Body.User.ID}`)
            if (doesExistUser.length) return res.send({ message: `User Already exist from this Email ${Body.User.Email}` })

            const [doesExistLoginName] = await DB.query(`select ID from user where LoginName = '${Body.User.LoginName}' and ID != ${Body.User.ID}`)
            if (doesExistLoginName.length) return res.send({ message: `LoginName Already exist from this LoginName ${Body.User.LoginName}` })
            ...
            const [updateCompany] = await DB.query(`update company set Name = '${Body.CompanyName}', MobileNo1='${Body.MobileNo1}', MobileNo2='${Body.MobileNo2}', PhoneNo='${Body.PhoneNo}', Address='${Body.Address}', Country='${Body.Country}', State='${Body.State}',City='${Body.City}', Website='${Body.Website}', GSTNo='${Body.GSTNo}', CINNo='${Body.CINNo}', LogoURL='${Body.LogoURL}', Remark='${Body.Remark}', SRemark='${Body.SRemark}', CAmount='${Body.CAmount}',  Plan=${Body.Plan}, Version='${Body.Version}', NoOfShops=${Body.NoOfShops}, EffectiveDate='${Body.EffectiveDate}', CancellationDate='${Body.CancellationDate}',EmailMsg='${Body.EmailMsg}', WhatsappMsg='${Body.WhatsappMsg}', IsPaidWhatsappMsg='${Body.IsPaidWhatsappMsg ? Body.IsPaidWhatsappMsg : 'false'}', WholeSale='${Body.WholeSale}', RetailPrice='${Body.RetailPrice}', Status=1, CompanyStatus=${Body.CompanyStatus}, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now(), Code = '${Body.Code ? Body.Code : 91}', PrimeMembership = '${Body.PrimeMembership}',PhotoClick = '${Body.PhotoClick}',CustomerCategory = '${Body.CustomerCategory}',EmployeeCommission = '${Body.EmployeeCommission}',LoginHistory = '${Body.LoginHistory}',DiscountSetting = '${Body.DiscountSetting}',Quotation = '${Body.Quotation}',ProductTransfer = '${Body.ProductTransfer}',BulkTransfer = '${Body.BulkTransfer}',PettyCash = '${Body.PettyCash}',LocationTracker = '${Body.LocationTracker}',StockCheck = '${Body.StockCheck}',RecycleBin = '${Body.RecycleBin}', AllExcelImport = '${Body.AllExcelImport}', OldBill = '${Body.OldBill}', SubDomainName = '${Body.SubDomainName ? Body.SubDomainName : ''}' where ID = ${Body.ID}`)

            console.log("Company Updated SuccessFUlly !!!");

            const [updateCompany2] = await connection.query(`update company set Name = '${Body.CompanyName}', MobileNo1='${Body.MobileNo1}', MobileNo2='${Body.MobileNo2}', PhoneNo='${Body.PhoneNo}', Address='${Body.Address}', Country='${Body.Country}', State='${Body.State}',City='${Body.City}', Website='${Body.Website}', GSTNo='${Body.GSTNo}', CINNo='${Body.CINNo}', LogoURL='${Body.LogoURL}', Remark='${Body.Remark}', SRemark='${Body.SRemark}', CAmount='${Body.CAmount}',  Plan=${Body.Plan}, Version='${Body.Version}', NoOfShops=${Body.NoOfShops}, EffectiveDate='${Body.EffectiveDate}', CancellationDate='${Body.CancellationDate}',EmailMsg='${Body.EmailMsg}', WhatsappMsg='${Body.WhatsappMsg}', IsPaidWhatsappMsg='${Body.IsPaidWhatsappMsg ? Body.IsPaidWhatsappMsg : 'false'}', WholeSale='${Body.WholeSale}', RetailPrice='${Body.RetailPrice}', Status=1, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now(), Code = '${Body.Code ? Body.Code : 91}', PrimeMembership = '${Body.PrimeMembership}',PhotoClick = '${Body.PhotoClick}',CustomerCategory = '${Body.CustomerCategory}',EmployeeCommission = '${Body.EmployeeCommission}',LoginHistory = '${Body.LoginHistory}',DiscountSetting = '${Body.DiscountSetting}',Quotation = '${Body.Quotation}',ProductTransfer = '${Body.ProductTransfer}',BulkTransfer = '${Body.BulkTransfer}',PettyCash = '${Body.PettyCash}',LocationTracker = '${Body.LocationTracker}',StockCheck = '${Body.StockCheck}',RecycleBin = '${Body.RecycleBin}',AllExcelImport = '${Body.AllExcelImport}',OldBill = '${Body.OldBill}', SubDomainName = '${Body.SubDomainName ? Body.SubDomainName : ''}' where ID = ${Body.ID}`)

            console.log("Company2 Updated SuccessFUlly !!!");


            const [updateUser] = await DB.query(`update user set Name = '${Body.User.Name}',DOB = '${Body.User.DOB}',Anniversary = '${Body.User.Anniversary}',PhotoURL = '${Body.User.PhotoURL}',MobileNo1 = '${Body.User.MobileNo1}',MobileNo2 = '${Body.User.MobileNo2}',PhoneNo = '${Body.User.PhoneNo}',Address = '${Body.User.Address}' where CompanyID = ${Body.ID} and UserGroup = 'CompanyAdmin'`)
            ...
            const [Company] = await DB.query(`select * from company where ID = ${Body.ID}`)
    ...
    updatePlan: async (req, res, next) => {
        ...
        try {
            ...
            const [updateCompany] = await DB.query(`update company set PrimeMembership = '${Body.PrimeMembership}',PhotoClick = '${Body.PhotoClick}',CustomerCategory = '${Body.CustomerCategory}',EmployeeCommission = '${Body.EmployeeCommission}',LoginHistory = '${Body.LoginHistory}',DiscountSetting = '${Body.DiscountSetting}',Quotation = '${Body.Quotation}',ProductTransfer = '${Body.ProductTransfer}',BulkTransfer = '${Body.BulkTransfer}',PettyCash = '${Body.PettyCash}',LocationTracker = '${Body.LocationTracker}',StockCheck = '${Body.StockCheck}',RecycleBin = '${Body.RecycleBin}',AllExcelImport = '${Body.AllExcelImport}',OldBill = '${Body.OldBill}', InsuranceModule = '${Body.InsuranceModule ? Body.InsuranceModule : false}', OrderRequest = '${Body.OrderRequest ? Body.OrderRequest : false}',  EcomShop = ${Body.EcomShop ? Body.EcomShop : 0}, EcomBilling = '${Body.EcomBilling ? Body.EcomBilling : false}', EcomSetting = '${Body.EcomSetting ? Body.EcomSetting : false}', EcomPower = '${Body.EcomPower ? Body.EcomPower : false}', EcomPaymentGateWay = '${Body.EcomPaymentGateWay ? Body.EcomPaymentGateWay : false}', EcomSignup = '${Body.EcomSignup ? Body.EcomSignup : false}', EcomLiveImage = '${Body.EcomLiveImage ? Body.EcomLiveImage : false}' where ID = ${Body.ID}`)
    ...
    delete: async (req, res, next) => {
        ...
        try {
            ...
            const [doesExist] = await DB.query(`select ID from company where Status = 1 and ID = ${Body.ID}`)
            ...
            const [deleteCompany] = await DB.query(`update company set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID}`)
            ...
            const [deleteUser] = await DB.query(`update user set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where CompanyID = ${Body.ID}`)
    ...
    activecompany: async (req, res, next) => {
        ...
        try {
            ...
            const [doesExist] = await DB.query(`select ID from company where Status = 0 and ID = ${Body.ID}`)
            ...
            const [activeCompany] = await DB.query(`update company set Status=1, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID}`)
    ...
    getCompanyById: async (req, res, next) => {
        ...
        try {
            ...
            const [Company] = await DB.query(`select company.*, company.Name as CompanyName, user.DOB, user.Anniversary, user.LoginName, user.PhotoURL, user.Name  from company left join user on user.CompanyID = company.ID where company.ID = ${Body.ID}`)

            const [User] = await DB.query(`SELECT * FROM user where companyID = ${Body.ID} and user.Status = 1`)
            ...
            if (Company[0].IsPaidWhatsappMsg === 'false') {
                Company[0].IsPaidWhatsappMsg = false
            } else {
                Company[0].IsPaidWhatsappMsg = true
            }
    ...
    LoginHistoryDetails: async (req, res, next) => {
        ...
        try {
            ...
            const [fetchCompanies] = await DB.query(`select company.ID AS ID, company.Name as Name, MobileNo1 as Mobile, supportmaster.Name AS CompanyStatus from company LEFT JOIN supportmaster ON supportmaster.ID  = company.CompanyStatus  where company.Status = 1 ${CompanyParam} ${CompanyStatusParam} order by ID desc`);
            if (fetchCompanies) {
                for (let item of fetchCompanies) {
                    const [countDetails] = await DB.query(`select count(ID) as Count from loginhistory where CompanyID = ${item.ID} ${DateParem}`)
                    let Obj = {
                        CompanyID: item.ID,
                        Name: item.Name,
    ...
    processProduct: async (req, res, next) => {
        ...
        try {
            ...
            for (let item of productData) {
                ...
                const [saveProduct] = await DB.query(`insert into product(SystemID,CompanyID, Name, HSNCode,GSTPercentage,GSTType,Status,CreatedBy,CreatedOn) values('${systemID}',${item.CompanyID}, '${item.Name}', '${item.HSNCode}',${item.GSTPercentage}, '${item.GSTType}', ${item.Status}, 0, now())`)
            }
