...
module.exports = {
    ...
    searchByBarcodeNo: async (req, res, next) => {
        ...
        try {
            ...
            if (PreOrder === "false") {
                ...
                qry = `SELECT COUNT(PurchaseDetailID) AS BarCodeCount, purchasedetailnew.GSTType, purchasedetailnew.GSTPercentage, purchasedetailnew.ProductName,purchasedetailnew.ProductTypeName,purchasedetailnew.ProductTypeID,purchasedetailnew.ProductExpDate, barcodemasternew.*,shop.Name as ShopName, shop.AreaName as AreaName,purchasedetailnew.UnitPrice, purchasedetailnew.PriceCut, purchasedetailnew.BaseBarCode, barcodemasternew.RetailPrice as RetailPrice, barcodemasternew.WholeSalePrice as WholeSalePrice   FROM barcodemasternew Left Join purchasedetailnew on purchasedetailnew.ID = barcodemasternew.PurchaseDetailID left join shop on shop.ID = barcodemasternew.ShopID Left join purchasemasternew on purchasemasternew.ID = purchasedetailnew.PurchaseID  WHERE CurrentStatus = "Available" AND Barcode = '${barCode}' ${searchParams}  and purchasedetailnew.Status = 1 and purchasedetailnew.PurchaseID != 0 and  purchasedetailnew.CompanyID = ${CompanyID} ${shopMode}`;
            } else {
    ...
    saveBill: async (req, res, next) => {
        ...
        try {
            ...
            let billingFlow = billMaseterData.BillingFlow ? billMaseterData.BillingFlow : 1;
            ...
            if (billDetailData.length) {
                for (let item of billDetailData) {
                    ...
                    if (manual === 0 && preorder === 0 && order === 0) {
                        ...
                        const data = await generatePreOrderProduct(CompanyID, shopid, item, LoggedOnUser, connection);
                    ...
                    if (ele.PreOrder === 1) {
                        ...
                        //         let [resultn] = await connection.query(`Update barcodemasternew set CurrentStatus = "Sold" , MeasurementID = '${ele.MeasurementID}', Family = '${ele.Family}',Optionsss = '${ele.Optionsss}', BillDetailID = ${ele.ID}, UpdatedBy=${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' Where ID = ${ele1.ID} and CompanyID = ${CompanyID} and ShopID = ${shopid}`);
                        //     })
                        // );
            ...
            response.data = {
                ...
                getBillById: await getBillById(bMasterID, CompanyID, db),
                billByCustomer: await billByCustomer(billMaseterData.CustomerID, bMasterID, CompanyID, shopid, db),
                paymentHistoryByMasterID: await paymentHistoryByMasterID(billMaseterData.CustomerID, bMasterID, CompanyID, shopid, db),
                getRewardBalance: await getRewardBalance(billMaseterData.CustomerID, `${billingFlow === 1 ? billMaseterData.InvoiceNo : billMaseterData.OrderNo}`, CompanyID, shopid, db),
            }
    ...
    saveBill2: async (req, res, next) => {
        ...
        try {
            ...
            if (billDetailData.length) {
                const invoiceNo = await generateInvoiceNo(CompanyID, shopid, billDetailData, billMaseterData, connection)
                billMaseterData.InvoiceNo = invoiceNo;
                productStatus = 'Pending'
            }
            ...
            if (service.length && !billDetailData.length) {
                billType = 0
                const invoiceNo = await generateInvoiceNoForService(CompanyID, shopid, service, billMaseterData, connection)
                billMaseterData.InvoiceNo = invoiceNo;
            }
            console.log("Invoice No ======>", billMaseterData.InvoiceNo);
            ...
            if (billDetailData.length) {
                ...
                for (const item of billDetailData) {
                    ...
                    if (manual === 0 && preorder === 0) {
                        ...
                        item.Barcode = Number(item.BaseBarCode)
                        // generate unique barcode
                        item.UniqueBarcode = await generateUniqueBarcodePreOrder(CompanyID, item, connection)
                        const data = await generatePreOrderProduct(CompanyID, shopid, item, LoggedOnUser, connection)
                        let [result] = await connection.query(
                            `insert into billdetail (BillID,CompanyID,ProductTypeID,ProductTypeName,ProductName,HSNCode,UnitPrice,PurchasePrice,Quantity,SubTotal,DiscountPercentage,DiscountAmount,GSTPercentage,GSTAmount,GSTType,TotalAmount,WholeSale, Manual, PreOrder,BaseBarCode,Barcode,Status, MeasurementID, Optionsss, Family, CreatedBy,CreatedOn, SupplierID, Remark, Warranty, ProductExpDate) values (${bMasterID}, ${CompanyID}, ${item.ProductTypeID},'${item.ProductTypeName}','${item.ProductName}', '${item.HSNCode}',${item.prod_UnitPrice},${item.PurchasePrice},${item.prod_Quantity},${item.prod_SubTotal}, ${item.prod_DiscountPercentage},${item.prod_DiscountAmount},${item.prod_GSTPercentage},${item.prod_GSTAmount},'${item.GSTType}',${item.prod_TotalAmount},${item.WholeSale},${manual}, ${preorder}, '${item.BaseBarCode}' ,'${item.Barcode}',1,'${item.MeasurementID}','${item.Option}','${item.Family}', ${LoggedOnUser}, '${req.headers.currenttime}', ${item.SupplierID}, '${item.Remark}', '${item.Warranty}', '${item.ProductExpDate}')`
                        );
    ...
    updateBillCustomer: async (req, res, next) => {
        ...
        try {
            ...
            response.data = {
                ...
                billByCustomer: await billByCustomer(billMaseterData.CustomerID, bMasterID, CompanyID, shopid, db),
                paymentHistoryByMasterID: await paymentHistoryByMasterID(billMaseterData.CustomerID, bMasterID, CompanyID, shopid, db),
                getRewardBalance: await getRewardBalance(billMaseterData.CustomerID, `${billMaseterData.BillingFlow === 1 ? billMaseterData.InvoiceNo : billMaseterData.OrderNo}`, CompanyID, shopid, db),
            }
    ...
    deleteProduct: async (req, res, next) => {
        ...
        try {
            ...
            const [fetchBill] = await connection.query(`select * from billmaster where CompanyID = ${CompanyID} and ShopID = ${shopid} and ID = ${billMaseterData.ID} and Status = 1 `)
            ...
            if (billDetailData) {
                ...
                const [fetchProductFromBillDetail] = await connection.query(`select * from billdetail where CompanyID = ${CompanyID} and ID = ${bDetail.ID}`);
                ...
                if (bDetail.OrderRequest === 1) {
                    const [findOrder] = await connection.query(`select * from orderrequest where CompanyID = ${CompanyID} and BillDetailID = ${bDetail.ID}`);
                ...
                const [delProduct] = await connection.query(`update billdetail set Status = 0, UpdatedBy=${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' where CompanyID = ${CompanyID} and ID = ${bDetail.ID} `)
                console.log(connected("Bill Detail Update SuccessFUlly !!!"));

                if (bDetail.Manual === 1) {
                    const [updateBarcode] = await connection.query(`update barcodemasternew set Status=0, BillDetailID=0 where CompanyID = ${CompanyID} and BillDetailID = ${bDetail.ID} and CurrentStatus = 'Not Available' limit ${bDetail.Quantity}`)
                ...
                if (bDetail.PreOrder === 1) {
                    const [fetchBarcode] = await connection.query(`select * from barcodemasternew where CompanyID = ${CompanyID} and BillDetailID = ${bDetail.ID} and PurchaseDetailID = 0 and  CurrentStatus = 'Pre Order' limit ${bDetail.Quantity}`);
                    ...
                    if (fetchBarcode.length && fetchBarcode.length === bDetail.Quantity) {
                        const [updateBarcode] = await connection.query(`update barcodemasternew set Status=0, BillDetailID=0 where CompanyID = ${CompanyID} and BillDetailID = ${bDetail.ID} and CurrentStatus = 'Pre Order' and PurchaseDetailID = 0 limit ${bDetail.Quantity}`)
                        console.log(connected("Barcode Update SuccessFUlly !!!"));
                    }
                    // if product is in preorder and has been purchased so we have to update for availlable
                    else if (!fetchBarcode.length) {
                        const [updateBarcode] = await connection.query(`update barcodemasternew set BillDetailID=0,CurrentStatus='Available' where CompanyID = ${CompanyID} and BillDetailID = ${bDetail.ID} and  PurchaseDetailID != 0 limit ${bDetail.Quantity}`)
                        console.log(connected("Barcode Update SuccessFUlly !!!"));
                    }
                ...
                if (bDetail.Manual === 0 && bDetail.PreOrder === 0 && bDetail.OrderRequest === 0) {
                    ...
                    [fetchbarcodeForPrice] = await connection.query(`select * from barcodemasternew where CompanyID = ${CompanyID} and CurrentStatus = 'Available' and  Barcode = '${billDetailData.Barcode}' limit 1`);

                    if (!fetchbarcodeForPrice.length) {
                        [fetchbarcodeForPrice] = await connection.query(`select * from barcodemasternew where CompanyID = ${CompanyID} and CurrentStatus = 'Sold' and  Barcode = '${billDetailData.Barcode}' and BillDetailID = ${bDetail.ID}  limit 1`);
                    }

                    //   console.log("fetchbarcodeForPrice ====>", fetchbarcodeForPrice);

                    const [updateBarcode] = await connection.query(`update barcodemasternew set CurrentStatus='Available', BillDetailID=0, RetailPrice = ${fetchbarcodeForPrice[0].RetailPrice} , WholeSalePrice = ${fetchbarcodeForPrice[0].WholeSalePrice} where CompanyID = ${CompanyID} and BillDetailID = ${bDetail.ID} and CurrentStatus = 'Sold'  limit ${bDetail.Quantity}`)
                    console.log(connected("Barcode Update SuccessFUlly !!!"));
            ...
            if (service) {
                ...
                const [delService] = await connection.query(`update billservice set Status = 0, UpdatedBy=${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' where CompanyID = ${CompanyID} and ID = ${bService.ID} `)
            ...
            if (doesCheckPayment.length === 1) {
                //  update payment
                const [updatePaymentMaster] = await connection.query(`update paymentmaster set PayableAmount = ${bMaster.TotalAmount} , PaidAmount = 0, UpdatedBy = ${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' where CompanyID = ${CompanyID} and ID = ${doesCheckPayment[0].PaymentMasterID}`)

                const [updatePaymentDetail] = await connection.query(`update paymentdetail set Amount = 0 , DueAmount = ${bMaster.TotalAmount}, UpdatedBy = ${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' where  CompanyID = ${CompanyID} and ID = ${doesCheckPayment[0].ID}`)
                console.log(connected("Payment Update SuccessFUlly !!!"));
            }
    ...
    updateProduct: async (req, res, next) => {
        ...
        try {
            ...
            const [fetchBill] = await connection.query(`select * from billmaster where CompanyID = ${CompanyID} and ShopID= ${shopid} and ID = ${bMasterID} and Status = 1 `)
        ...
        try {
            ...
            ejs.renderFile(path.join(appRoot, './views/', formatName), { data: printdata }, (err, data) => {
                if (err) {
                    ...
                    } else if (formatName == 'optometric.ejs' || formatName == 'ajanta.ejs') {
                        options = {
                            format: "A5",
                            orientation: "portrait",
                        };
    ...
    getSalereportsDetailExport: async (req, res, next) => {
        ...
        try {
            ...
            let [datum] = await connection.query(`SELECT SUM(billdetail.Quantity) as totalQty, SUM(billdetail.GSTAmount) as totalGstAmount, SUM(billdetail.TotalAmount) as totalAmount, SUM(billdetail.DiscountAmount) as totalDiscount, SUM(billdetail.SubTotal) as totalUnitPrice  FROM billmaster LEFT JOIN customer ON customer.ID = billmaster.CustomerID left join user on user.ID = billmaster.Employee LEFT JOIN billdetail ON billdetail.BillID = billmaster.ID  LEFT JOIN shop ON shop.ID = billmaster.ShopID WHERE billdetail.Status = 1  ${searchString} AND billdetail.CompanyID = ${CompanyID} ` + Parem)
    ...
    saveConvertPurchase: async (req, res, next) => {
        ...
        try {
            ...
            for (const item of PurchaseDetail) {
                ...
                // generate unique barcode
                item.UniqueBarcode = await generateUniqueBarcode(CompanyID, supplierId, item, connection)
    ...
    cashcollectionreport: async (req, res, next) => {
        ...
        try {
            ...
            let qry = `select paymentmaster.CustomerID, paymentmaster.ShopID, paymentmaster.PaymentMode, paymentmaster.PaymentDate, paymentmaster.CardNo, paymentmaster.PaymentReferenceNo, paymentmaster.PayableAmount, paymentdetail.Amount, paymentdetail.DueAmount, billmaster.InvoiceNo, billmaster.BillDate,billmaster.DeliveryDate, billmaster.PaymentStatus, billmaster.TotalAmount, shop.Name as ShopName, shop.AreaName, customer.Name as CustomerName, customer.MobileNo1, paymentmaster.CreditType from paymentdetail left join paymentmaster on paymentmaster.ID = paymentdetail.PaymentMasterID left join billmaster on billmaster.ID = paymentdetail.BillMasterID left join shop on shop.ID = paymentmaster.ShopID left join customer on customer.ID = paymentmaster.CustomerID where  paymentmaster.CompanyID = ${CompanyID} and paymentdetail.PaymentType IN ( 'Customer', 'Customer Credit', 'Manual Customer Credit' ) and paymentmaster.CreditType = 'Credit' and paymentmaster.PaymentMode != 'Payment Initiated' and billmaster.Status = 1   ${shop} ${paymentStatus} ${paymentType} ` + Date + ` order by paymentdetail.BillMasterID desc`;
    ...
    saveDiscountSetting: async (req, res, next) => {
        ...
        try {
            ...
            connection = await db.getConnection();
            const checkExist = await doesExistDiscoutSetting(CompanyID, ShopID, req.body, connection)
    ...
    updateDiscountSetting: async (req, res, next) => {
        ...
        try {
            ...
            const checkExist = await doesExistDiscoutSettingUpdate(CompanyID, ShopID, ID, req.body, connection)
            ...
            if (fetchProductReturnDetail.length) {
                for (let item of fetchProductReturnDetail) {
                    ...
                    if (item.Manual === 0 && item.PreOrder === 0 && item.OrderRequest === 0) {
                        ...
                        const var_update_c_report = await update_c_report(CompanyID, shopid, 0, 0, 0, 0, item.Quantity, 0, 0, 0, 0, 0, 0, 0, 0, req.headers.currenttime, connection)

                        const totalAmount = await getTotalAmountByBarcode(CompanyID, item.Barcode, connection)

                        const var_amt_update_c_report = await amt_update_c_report(CompanyID, shopid, 0, 0, 0, 0, item.Quantity * Number(totalAmount), 0, 0, 0, 0, 0, 0, 0, 0, req.headers.currenttime, connection)
                    }
                }
            }
        ...
        try {
            ...
            if (barCodeData.length) {
                for (let item of barCodeData) {
                    ...
                    if (item.ProductStatus === "Order Request" && (item.AppliedPaidAmountPercentage === 0 || item.AppliedPaidAmountPercentage === "0")) {
                        ...
                        const paidAmount = Number((findBillDetails[0]?.TotalAmount - findBillDetails[0]?.DueAmount).toFixed(2));
    ...
    orderformrequestfilter: async (req, res, next) => {
        let connection;
        try {
            const response = { data: null, success: true, message: "" }
            ...
            qry = `select orderrequest.AppliedPaidAmountPercentage, orderrequest.ID, orderrequest.BillDetailID, orderrequest.ProductName,orderrequest.ProductTypeID, orderrequest.OrderRequestShopID, orderrequest.ShopID as OrderInvoiceShopID, orderrequest.ProductTypeName, orderrequest.HSNCode, orderrequest.DocNo,  orderrequest.Quantity, 0 as SaleQuantity, orderrequest.ProductStatus, orderrequest.Barcode, orderrequest.BaseBarCode, billmaster.InvoiceNo,  billmaster.BillDate, customer.Name as CustomerName, customer.MobileNo1 as CustomerMobileNo, CONCAT(ss.Name, '(', ss.AreaName, ')') AS InvoiceShopName, CONCAT(ss2.Name, '(', ss2.AreaName, ')') AS OrderRequestShopName, billdetail.MeasurementID, IFNULL(supplier.Name, 'NA') AS SupplierName from orderrequest left join billmaster on billmaster.ID = orderrequest.BillMasterID left join customer on customer.ID = billmaster.CustomerID left join shop AS ss on ss.ID = orderrequest.ShopID left join shop AS ss2 on ss2.ID = orderrequest.OrderRequestShopID left join billdetail on billdetail.ID = orderrequest.BillDetailID left join supplier ON supplier.ID = orderrequest.SupplierID where orderrequest.Status = 1 and orderrequest.IsStock = 0 and orderrequest.SupplierID != 0 and orderrequest.CompanyID = ${CompanyID}  ${Params}`;
    ...
    orderformsubmitRxCancel: async (req, res, next) => {
        ...
        try {
            ...
            const [fetchOrderRequest] = await connection.query(`select * from orderrequest where Status = 1 and ID = ${ID} and CompanyID = ${CompanyID} and IsStock = 0`);
            ...
            const [update] = await connection.query(`update orderrequest set ProductStatus = 'Order Request', SupplierID = 0 where ID = ${ID} and CompanyID = ${CompanyID} and IsStock = 0`)
    ...
    orderformrequestRemovePercentageRange: async (req, res, next) => {
        let connection;
        try {
            const response = { data: null, success: true, message: "" }
            const { ID } = req.body;
            ...
            const [update] = await connection.query(`update orderrequest set AppliedPaidAmountPercentage = 1 where ID = ${ID} and CompanyID = ${CompanyID}`)

            response.message = "Order Percentage Range Removed successfully";
            response.data = {}
            return res.send(response);
    ...
    orderformsubmitRxDocNo: async (req, res, next) => {
        let connection;
        try {
            const response = { data: null, success: true, message: "" }
            const { ID, DocNo } = req.body;
            const CompanyID = req.user.CompanyID ? req.user.CompanyID : 0;
            const shopid = await shopID(req.headers) || 0;
            // const db = await dbConfig.dbByCompanyID(CompanyID);
            const db = req.db;
            ...
            const [update] = await connection.query(`update orderrequest set DocNo = '${DocNo}' where ID = ${ID} and CompanyID = ${CompanyID} and IsStock = 0`)

            response.message = "Order Rx DocNo Save successfully";
            response.data = {}
            return res.send(response);
            ...
            for (let item of OrderList) {
                ...
                if (fetchInvoiceMaster.length) {
                    if (fetchInvoiceMaster[0].BillType === 1) {
                        ...
                        inv = await generateInvoiceNo(CompanyID, fetchInvoiceMaster[0].ShopID, [{ WholeSale: fetchInvoiceDetail[0].WholeSale }], { ID: null }, connection)

                    }
                    if (fetchInvoiceMaster[0].BillType === 0) {
                        inv = await generateInvoiceNoForService(CompanyID, fetchInvoiceMaster[0].ShopID, [], { ID: null }, connection)
                    }
                    ...
                    if (tempAmount !== 0) {
                        ...
                        if (item.PaymentStatus === "Paid") {
                            ...
                            if (fetchInvoiceMaster.length && fetchInvoiceMaster[0].BillType === 1) {
                                const [fetchInvoiceDetail] = await connection.query(`select * from billdetail where BillID = ${item.ID} and CompanyID = ${CompanyID} limit 1 `);
                                inv = await generateInvoiceNo(CompanyID, ShopID, [{ WholeSale: fetchInvoiceDetail[0].WholeSale }], { ID: null }, connection)
                            }
                            if (fetchInvoiceMaster.length && fetchInvoiceMaster[0].BillType === 0) {
                                inv = await generateInvoiceNoForService(CompanyID, ShopID, [], { ID: null }, connection)
                            }
                        ...
                        if (PaymentMode.toUpperCase() === "CASH") {
                            ...
                            const update_pettycash = update_pettycash_report(CompanyID, ShopID, "Sale", item.Amount, "CashCounter", req.headers.currenttime, connection)
                        ...
                        if (item.PaymentStatus === "Paid") {
                            ...
                            const saveReward = await reward_master(CompanyID, ShopID, billRows[0].CustomerID, item.InvoiceNo, fetchBillMaster[0].Amount, "credit", LoggedOnUser, connection) //CompanyID, ShopID, CustomerID, InvoiceNo, PaidAmount, CreditType, LoggedOnUser
                        }
    ...
    getInsuranceReport: async (req, res, next) => {
        ...
        try {
            ...
            qry = `SELECT i.ID, i.BillMasterID, i.InsuranceCompanyName, i.PolicyNumber, i.Remark, i.Other, i.ClaimAmount, i.ApprovedAmount, i.PaidAmount, i.RemainingAmount, i.CreatedBy, i.CreatedOn, i.PaymentStatus, i.RequestDate, i.ApproveDate,  CASE WHEN bm.BillingFlow = 1 THEN bm.InvoiceNo ELSE bm.OrderNo END AS BillNumber, c.Idd AS CustomerID, CASE WHEN c.Title IS NULL OR c.Title = '' THEN c.Name ELSE CONCAT(c.Title, ' ', c.Name) END AS CustomerName, CASE WHEN c.MobileNo1 IS NOT NULL AND c.MobileNo1 <> '' THEN c.MobileNo1 WHEN c.PhoneNo IS NOT NULL AND c.PhoneNo <> '' THEN c.PhoneNo ELSE '' END AS Mobile, bm.DueAmount as BillDueAmount,CONCAT(COALESCE(shop.Name, ''), CASE WHEN shop.Name IS NOT NULL AND shop.AreaName IS NOT NULL THEN '(' ELSE '' END, COALESCE(shop.AreaName, ''), CASE WHEN shop.Name IS NOT NULL AND shop.AreaName IS NOT NULL THEN ')' ELSE '' END) AS ShopName FROM insurance i LEFT JOIN billmaster bm ON bm.ID = i.BillMasterID LEFT JOIN customer c ON c.ID = bm.CustomerID LEFT JOIN shop on shop.ID = i.ShopID WHERE i.CompanyID = ${CompanyID} ${Parem}`
        ...
        if (fetchBill.length) {
            for (let item of fetchBill) {
                ...
                if (fetch && fetch.length) {
                    ...
                    if (billMaseterData) {
                        const saveCommission = await generateCommission(CompanyID, 'Doctor', UserID, item.ID, billMaseterData[0], item.CreatedBy, connection)
                    }
