    ...
    updateNew: async (req, res, next) => {
        ...
        try {
                ...
                for (const item of purchaseDetail) {
                    if (!item.ID) {
                        ...
                        for (let j = 0; j < item.Quantity; j++) {
                            await connection.query(
                                `INSERT INTO barcodemasternew (CompanyID, ShopID, PurchaseDetailID, GSTType, GSTPercentage, BarCode,  AvailableDate, CurrentStatus, RetailPrice, RetailDiscount,  MultipleBarcode, ForWholeSale, WholeSalePrice, WholeSaleDiscount, TransferStatus, TransferToShop, Status, CreatedBy, CreatedOn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`,
                                [
                                    CompanyID,
                                    shopid,
            ...
            if (Charge && Charge.length) {
                for (const c of Charge) {
                    if (!c.ID) {
                        shouldUpdatePayment = true;
                        await connection.query(
                            `INSERT INTO purchasecharge (PurchaseID, ChargeType, CompanyID, Description, Amount, GSTPercentage, GSTAmount, GSTType, TotalAmount, Status, CreatedBy, CreatedOn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`,
                            [
                                purchase.ID,
                                c.ChargeType,
                                CompanyID,
            ...
            if (shouldUpdatePayment && doesCheckPayment.length) {
                await connection.query(
                    `UPDATE paymentmaster SET PayableAmount = ?, PaidAmount = 0, UpdatedBy = ?, UpdatedOn = ? WHERE ID = ? AND CompanyID = ?`,
                    [PurchaseMaster.TotalAmount, LoggedOnUser, req.headers.currenttime, doesCheckPayment[0].PaymentMasterID, CompanyID]
                );

                await connection.query(
                    `UPDATE paymentdetail  SET BillID = ?, Amount = 0, DueAmount = ?, UpdatedBy = ?, UpdatedOn = ? WHERE ID = ? AND CompanyID = ?`,
        ...
        try {
            ...
            if (purchaseDetail.length) {
                for (const item of purchaseDetail) {
                    ...
                    const [savePurchaseDetail] = await connection.query(`insert into purchasedetailnew(PurchaseID,CompanyID,ProductName,ProductTypeID,ProductTypeName,UnitPrice, Quantity,SubTotal,DiscountPercentage,DiscountAmount,GSTPercentage, GSTAmount,GSTType,TotalAmount,RetailPrice,WholeSalePrice,PriceCut,MultipleBarCode,WholeSale,BaseBarCode,Ledger,Status,NewBarcode,ReturnRef,BrandType,UniqueBarcode,ProductExpDate,Checked,BillDetailIDForPreOrder,CreatedBy,CreatedOn)values(${savePurchase.insertId},${CompanyID},'${item.ProductName}',${item.ProductTypeID},'${item.ProductTypeName}', ${item.UnitPrice},${item.Quantity},${item.SubTotal},${item.DiscountPercentage},${item.DiscountAmount},${item.GSTPercentage},${item.GSTAmount},'${item.GSTType}',${item.TotalAmount},${item.RetailPrice},${item.WholeSalePrice},${item.PriceCut ? item.PriceCut : 0},${item.Multiple},${item.WholeSale},'${baseBarCode}',${item.Ledger},1,'${baseBarCode}',0,${item.BrandType},'${item.UniqueBarcode}','${item.ProductExpDate}',0,0,${LoggedOnUser},'${req.headers.currenttime}')`)
            ...
            if (purchaseDetail.length) {
                for (const item of purchaseDetail) {
                    if (item.ID === null) {
                        ...
                        const [savePurchaseDetail] = await connection.query(`insert into purchasedetailnew(PurchaseID,CompanyID,ProductName,ProductTypeID,ProductTypeName,UnitPrice, Quantity,SubTotal,DiscountPercentage,DiscountAmount,GSTPercentage, GSTAmount,GSTType,TotalAmount,RetailPrice,WholeSalePrice,PriceCut,MultipleBarCode,WholeSale,BaseBarCode,Ledger,Status,NewBarcode,ReturnRef,BrandType,UniqueBarcode,ProductExpDate,Checked,BillDetailIDForPreOrder,CreatedBy,CreatedOn)values(${purchase.ID},${CompanyID},'${item.ProductName}',${item.ProductTypeID},'${item.ProductTypeName}', ${item.UnitPrice},${item.Quantity},${item.SubTotal},${item.DiscountPercentage},${item.DiscountAmount},${item.GSTPercentage},${item.GSTAmount},'${item.GSTType}',${item.TotalAmount},${item.RetailPrice},${item.WholeSalePrice},${item.PriceCut ? item.PriceCut : 0},${item.Multiple},${item.WholeSale},'${baseBarCode}',${item.Ledger},1,'${baseBarCode}',0,${item.BrandType},'${item.UniqueBarcode}','${item.ProductExpDate}',0,0,${LoggedOnUser},'${req.headers.currenttime}')`)
    ...
    AllPrintBarcode: async (req, res, next) => {
        ...
        try {
            ...
            printdata.forEach(ele => {
                ...
                if (ele.ProductTypeName !== 'SUNGLASSES' && ele.ProductTypeName !== 'SUNGLASS' && ele.ProductTypeName !== 'Frames#1' && ele.ProductTypeName !== 'CONTACT LENS' && ele.ProductTypeName !== 'READING FRAME' && ele.ProductTypeName !== 'WATCH') {
                ...
                if (ProductTypess !== undefined) {
                    ele.ProductTypess = ProductTypess.substring(0, 20)
                } else {
                ...
                ejs.renderFile(
                    ...
                    async (err, data) => {
                        if (err) {
                            ...
                            else if (printdata.CompanyBarcode == 100) {
                                options = {
                                    "height": "0.85in",
                                    "width": "131mm",
                                    timeout: 600000,
 ...
 format: "A4",
    ...
    getPurchasereports: async (req, res, next) => {
        ...
        try {
            ...
            let [datum] = await connection.query(`SELECT SUM(purchasemasternew.Quantity) as totalQty, SUM(purchasemasternew.GSTAmount) as totalGstAmount, SUM(purchasemasternew.TotalAmount) as totalAmount, SUM(purchasemasternew.DiscountAmount) as totalDiscount, SUM(purchasemasternew.SubTotal) as totalUnitPrice  FROM purchasemasternew LEFT JOIN shop ON shop.ID = purchasemasternew.ShopID LEFT JOIN supplier ON supplier.ID = purchasemasternew.SupplierID WHERE purchasemasternew.Status = 1 and supplier.Name != 'PreOrder Supplier' AND purchasemasternew.CompanyID = ${CompanyID}  ` + Parem)

            // let [datum] = await connection.query(`SELECT SUM(purchasemasternew.Quantity) as totalQty, SUM(purchasemasternew.GSTAmount) as totalGstAmount, SUM(purchasemasternew.TotalAmount) as totalAmount, SUM(purchasemasternew.DiscountAmount) as totalDiscount, SUM(purchasemasternew.SubTotal) as totalUnitPrice  FROM purchasedetailnew INNER JOIN purchasemasternew ON purchasemasternew.ID = purchasedetailnew.PurchaseID LEFT JOIN shop ON shop.ID = purchasemasternew.ShopID LEFT JOIN supplier ON supplier.ID = purchasemasternew.SupplierID LEFT JOIN product ON product.ID = purchasedetailnew.ProductTypeID WHERE purchasedetailnew.Status = 1 and purchasemasternew.Status = 1 and supplier.Name != 'PreOrder Supplier' AND purchasedetailnew.CompanyID = ${CompanyID}  ` + Parem)
    ...
    getProductInventoryReport: async (req, res, next) => {
        ...
        try {
            ...
            qry = `SELECT COUNT(barcodemasternew.ID) AS Count, purchasedetailnew.BrandType, purchasedetailnew.ID as PurchaseDetailID , purchasedetailnew.UnitPrice, purchasedetailnew.Quantity, purchasedetailnew.ID, purchasedetailnew.DiscountAmount, purchasedetailnew.TotalAmount, supplier.Name AS SupplierName, shop.Name AS ShopName, shop.AreaName AS AreaName, purchasedetailnew.ProductName, purchasedetailnew.ProductTypeName, purchasedetailnew.UnitPrice, purchasedetailnew.SubTotal, purchasedetailnew.DiscountPercentage, purchasedetailnew.GSTPercentage as GSTPercentagex, purchasedetailnew.GSTAmount, purchasedetailnew.GSTType as GSTTypex, purchasedetailnew.WholeSalePrice, purchasemasternew.InvoiceNo, purchasemasternew.PurchaseDate, purchasemasternew.PaymentStatus,  barcodemasternew.*, purchasemasternew.SupplierID, purchasedetailnew.ProductNameArray, purchasedetailnew.Ecom FROM barcodemasternew LEFT JOIN purchasedetailnew ON purchasedetailnew.ID = barcodemasternew.PurchaseDetailID  LEFT JOIN purchasemasternew ON purchasemasternew.ID = purchasedetailnew.PurchaseID LEFT JOIN supplier ON supplier.ID = purchasemasternew.SupplierID  LEFT JOIN shop ON shop.ID = barcodemasternew.ShopID  where barcodemasternew.CompanyID = ${CompanyID} ${searchString} AND purchasedetailnew.Status = 1 and supplier.Name != 'PreOrder Supplier'  ` + Parem + " Group By barcodemasternew.PurchaseDetailID, barcodemasternew.ShopID" + " HAVING barcodemasternew.Status = 1";
            let [data] = await connection.query(qry);
    ...
    updatePhysicalStockProduct2: async (req, res, next) => {
        ...
        try {
            ...
            for (let item of xDetail) {
                // console.log(`update physicalstockcheckdetail set AvailableQty=${item.AvailableQty},PhysicalAvailableQty=${item.PhysicalAvailableQty},QtyDiff=${item.QtyDiff} where ID = ${item.ID} and CompanyID = ${CompanyID}`);
    ...
    orderPurchaseSoldProcess: async (req, res, next) => {
        ...
        try {
            ...
            if (!PurchaseMaster.SupplierID || PurchaseMaster.SupplierID === undefined || PurchaseMaster.SupplierID === 0) return res.send({ message: "Invalid SupplierID Data" });
            ...
            if (!PurchaseMaster?.Quantity || PurchaseMaster?.Quantity === null || PurchaseMaster.Quantity == 0) return res.send({ message: "Invalid Query Data Quantity" });
            ...
            for (const pd of purchaseDetail) {
                ...
                let saleListData = [{ "ProductName": `${item.ProductName}`, "Barcode": `${baseBarCode}`, "SaleQty": `${item.Quantity}`, "AvailableQty": 0 }]
                ...
                if (detailDataForBarCode.length) {
                    for (const item of detailDataForBarCode) {
                        ...
                        count = item.Quantity;
                        for (let j = 0; j < count; j++) {
                            const [saveBarcode] = await connection.query(`insert into barcodemasternew(CompanyID, ShopID, PurchaseDetailID, BillDetailID, OrderID, GSTType, GSTPercentage, BarCode, AvailableDate, CurrentStatus, RetailPrice, RetailDiscount, MultipleBarcode, ForWholeSale, WholeSalePrice, WholeSaleDiscount, TransferStatus, TransferToShop, Status, CreatedBy, CreatedOn)values(${CompanyID},${shopid},${item.ID},${BillDetailID}, ${OrderID},'${item.GSTType}',${item.GSTPercentage}, '${barcode}','${req.headers.currenttime}','${currentStatus}', ${item.RetailPrice},0,${item.MultipleBarCode},${item.WholeSale},${item.WholeSalePrice},0,'',0,1,${LoggedOnUser}, '${req.headers.currenttime}')`)
                        }
