...
// raguler grid
  ...
  openModalGrid(content001: any, data: any) {
      ...
      this.AxisAddHide = this.inventory.ProductName.includes(`SINGLE VISION`) ? false : true;
  ...
  exportAsXLSXlensGrid(): void {
    let element = document.getElementById('lensGridExcel');
    const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(element);
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
    XLSX.writeFile(wb, 'Lens-Grid-Stock.xlsx');
  }
