...
export class FitterPoComponent implements OnInit {
 ...
 myControl = new FormControl('All');
  ...
  Search(mode: any) {
    ...
    if ( this.data.FromDate !== '' && this.data.FromDate !== null && this.data.ToDate !== '' && this.data.ToDate !== null && this.data.CustomerID === 'All') {
  ...
  if (this.companySetting.BillingFlow != 1) {
    ...
    Parem += `
      AND DATE_FORMAT(billmaster.BillDate, "%Y-%m-%d")
