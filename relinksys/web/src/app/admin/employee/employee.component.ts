...
export class EmployeeComponent implements OnInit {
  ...
  data: any = {
    ...
    CommissionValue: 0, CommissionValueNB: 0, DiscountPermission: false, SalePermission: false, IsGetReport: false, ExcelPermission :false,PaymentView:false
  };
    ...
    const subs: Subscription = this.es.getUserById(this.id).subscribe({
      next: (res: any) => {
        ...
        if (res.success) {
          ...
          this.data.ExcelPermission = this.data.ExcelPermission === 'true';
          this.data.PaymentView = this.data.PaymentView === 'true';
