...
async function sendOpticalGuruMail() {
  try {
    // Helper (define it if not already imported)
...
<body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px; color: #333;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; margin: auto; background-color: #ffffff; padding: 20px; border: 1px solid #ddd;">
    <tr>
      <td>
        <p style="font-size: 16px;">Dear <strong>${toTitleCase('Rahul')}</strong>,</p>
    ...
    // ✅ Use promise-style instead of callback (if supported)
    const resp = await Mail.sendMailForOwn(emailData);
    console.log({ success: true, message: 'Data saved & Mail sent successfully', response: resp });

  } catch (error) {
    console.error('❌ Error in sendOpticalGuruMail:', error);
  }
}
