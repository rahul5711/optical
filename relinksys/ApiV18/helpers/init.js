...
const c_report_init = async () => {
    ...
    try {
        DB = await mysql2.pool.getConnection();
        let date = moment(new Date('2025-11-08')).format("YYYY-MM-DD")
        const [company] = await DB.query(`select ID, Name from company where Status = 1 and ID = 408`)
        let result = []
        if (company) {
...
async function eyeHold() {
    try {
        ...
        const bodyData = {
            ...
            "campaignName": `opticalguru_customer_bill_advance_new`,
            "destination": `919766666248`,
            "userName": "eyehold",
            "templateParams": [
                `Santosh Ji`,
                `Relinksys`,
            ...
            "media": {
                "url": `https://theopticalguru.relinksys.com/uploads/Bill-1020425-128.pdf`,
                "filename": `Bill-1020425-128`
            },
            "buttons": [],
            "carouselCards": [],
            "location": {},
