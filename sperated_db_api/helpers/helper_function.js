...
module.exports = {
  ...
  Idd: async (req, res, next) => {
    ...
    const [customer] = await db.query(`select ID from customer where CompanyID = ${CompanyID}  ${shop}`);
  ...
  generateVisitNo: async (CompanyID, CustomerID, TableName) => {
    ...
    const [visitNo] = await db.query(`select ID from ${TableName} where CompanyID = ${CompanyID} and CustomerID = ${CustomerID}`)
  ...
  generateBarcode: async (CompanyID, BarcodeType) => {
    ...
    const [barcode] = await db.query(`select barcode.${BarcodeType} from barcode where Status = 1 and CompanyID=${CompanyID}`);
    if (BarcodeType === 'SB') {
      const [updateBarcode] = await db.query(`update barcode set ${BarcodeType} = ${Number(barcode[0].SB) + 1}, UpdatedOn = now() where CompanyID=${CompanyID}`)
      return Number(barcode[0].SB)
    } else if (BarcodeType === 'PB') {
      const [updateBarcode] = await db.query(`update barcode set ${BarcodeType} = ${Number(barcode[0].PB) + 1}, UpdatedOn = now() where CompanyID=${CompanyID}`)
      return Number(barcode[0].PB)
    } else if (BarcodeType === 'MB') {
      const [updateBarcode] = await db.query(`update barcode set ${BarcodeType} = ${Number(barcode[0].MB) + 1}, UpdatedOn = now() where CompanyID=${CompanyID}`)
  ...
  doesExistProduct: async (CompanyID, Body) => {
    ...
    const [barcode] = await db.query(qry)
  ...
  doesExistDiscoutSetting: async (CompanyID, ShopID, Body) => {
    ...
    const [fetch] = await db.query(`SELECT ID FROM discountsetting WHERE ProductName = '${Body.ProductName}' AND ProductTypeID = '${Body.ProductTypeID}' AND CompanyID = '${CompanyID}' AND ShopID = '${ShopID}' AND Status = 1`);
    if (fetch.length) {
      return true
    }
    return false
  ...
  doesExistDiscoutSettingUpdate: async (CompanyID, ShopID, ID, Body) => {
    ...
    const [fetch] = await db.query(`SELECT ID FROM discountsetting WHERE ProductName = '${Body.ProductName}' AND ProductTypeID = '${Body.ProductTypeID}' AND CompanyID = '${CompanyID}' AND ShopID = '${ShopID}' AND Status = 1 and ID != ${ID}`);
    if (fetch.length) {
      return true
    }
    return false
  ...
  generateUniqueBarcode: async (CompanyID, SupplierID, Body) => {
    ...
    const [fetchcompanysetting] = await db.query(`select year, month, partycode, type from companysetting where Status = 1 and CompanyID = ${CompanyID} `)
    ...
    const [fetchSupplier] = await db.query(`select ID, Sno from supplier where Status = 1 and CompanyID = ${CompanyID} and ID = ${SupplierID}`)
  ...
  generateUniqueBarcodePreOrder: async (CompanyID, Body) => {
    ...
    // const fetchSupplier = await db.query(`select * from supplier where Status = 1 and CompanyID = ${CompanyID} and ID = ${SupplierID}`)

    const [fetchSupplier] = await db.query(`select ID, Sno  from supplier where CompanyID = ${CompanyID} and Name = 'PreOrder Supplier'`)
  ...
  gstDetail: async (CompanyID, PurchaseID) => {
    ...
    if (gstTypes.length) {
      for (const item of gstTypes) {
        let [value] = await db.query(`select SUM(GSTAmount) as Amount, GSTType from purchasedetailnew where CompanyID = ${CompanyID} and PurchaseID = ${PurchaseID} and Status = 1 and GSTType = '${item.Name}'`)
    ...
    if (gstTypes.length) {
      for (const item of gstTypes) {
        let [value] = await db.query(`select SUM(GSTAmount) as Amount, GSTType from purchasecharge where CompanyID = ${CompanyID} and PurchaseID = ${PurchaseID} and Status = 1 and GSTType = '${item.Name}'`)
        value = JSON.parse(JSON.stringify(value)) || []
        if (value.length) {
          if ((item.Name).toUpperCase() === 'CGST-SGST') {
            values2.push(
  ...
  gstDetailQuotation: async (CompanyID, PurchaseID) => {
    ...
    if (gstTypes.length) {
      for (const item of gstTypes) {
        let [value] = await db.query(`select SUM(GSTAmount) as Amount, GSTType from purchasedetailnewpo where CompanyID = ${CompanyID} and PurchaseID = ${PurchaseID} and Status = 1 and GSTType = '${item.Name}'`)
        value = JSON.parse(JSON.stringify(value)) || []
        if (value.length) {
          if ((item.Name).toUpperCase() === 'CGST-SGST') {
            values.push(
  ...
  gstDetailBill: async (CompanyID, BillID) => {
    ...
    if (gstTypes.length) {
      for (const item of gstTypes) {
        let [value] = await db.query(`select SUM(GSTAmount) as Amount, GSTType from billdetail where CompanyID = ${CompanyID} and BillID = ${BillID} and Status = 1 and GSTType = '${item.Name}'`)
        value = JSON.parse(JSON.stringify(value)) || []
        if (value.length) {
          if ((item.Name).toUpperCase() === 'CGST-SGST') {
            values.push(
    ...
    if (gstTypes.length) {
      for (const item of gstTypes) {
        let [value] = await db.query(`select SUM(GSTAmount) as Amount, GSTType from billservice where CompanyID = ${CompanyID} and BillID = ${BillID} and Status = 1 and GSTType = '${item.Name}'`)
        value = JSON.parse(JSON.stringify(value)) || []
        if (value.length) {
          if ((item.Name).toUpperCase() === 'CGST-SGST') {
            values2.push(
  ...
  generateInvoiceNo: async (CompanyID, ShopID, billDetailData, billMaseterData) => {
    ...
    const [billShopWise] = await db.query(`select ID, BillShopWise from shop where CompanyID = ${CompanyID} and ID = ${ShopID} and Status = 1`);
    ...
    if (billShopWiseBoolean) {
      [lastInvoiceID] = await db.query(`select Retail, WholeSale  from invoice WHERE CompanyID = '${CompanyID}' and ShopID = ${ShopID}`);
      ...
      const [update] = await db.query(`update invoice set Retail = ${updateDatum.Retail}, WholeSale = ${updateDatum.WholeSale}, UpdatedOn = now() WHERE CompanyID = '${CompanyID}' and ShopID = ${ShopID}`)

    } else {
      [lastInvoiceID] = await db.query(`select Retail, WholeSale from invoice WHERE CompanyID = '${CompanyID}' and ShopID = 0`);
      ...
      const [update] = await db.query(`update invoice set Retail = ${updateDatum.Retail}, WholeSale = ${updateDatum.WholeSale}, UpdatedOn = now() WHERE CompanyID = '${CompanyID}' and ShopID = 0`)
    }

    const [shopDetails] = await db.query(`select ID, Sno from shop where CompanyID = ${CompanyID} and ID = ${ShopID} and Status = 1`)
  ...
  generateInvoiceNoForService: async (CompanyID, ShopID, billDetailData, billMaseterData) => {
    ...
    if (billShopWiseBoolean) {
      [lastInvoiceID] = await db.query(`select Service from invoice WHERE CompanyID = '${CompanyID}' and ShopID = ${ShopID}`);

      const updateDatum = {
        Service: lastInvoiceID[0].Service + 1
      }

      const [update] = await db.query(`update invoice set Service = ${updateDatum.Service}, UpdatedOn = now() WHERE CompanyID = '${CompanyID}' and ShopID = ${ShopID}`)

    } else {
      [lastInvoiceID] = await db.query(`select Service from invoice WHERE CompanyID = '${CompanyID}' and ShopID = 0`);

      const updateDatum = {
        Service: lastInvoiceID[0].Service + 1
      }

      const [update] = await db.query(`update invoice set Service = ${updateDatum.Service}, UpdatedOn = now() WHERE CompanyID = '${CompanyID}' and ShopID = 0`)
    }
  ...
  generateInvoiceNo2: async (CompanyID, ShopID, billDetailData, billMaseterData) => {
    ...
    const [billShopWise] = await db.query(`select ID, BillShopWise from shop where CompanyID = ${CompanyID}`);
    if (billShopWise.length) {
      if (billShopWise[0].BillShopWise == true || billShopWise[0].BillShopWise == "true") {
        billShopWiseBoolean = true
      } else {
    ...
    // and InvoiceNo LIKE '${newInvoiceID}%'
    if (billShopWiseBoolean) {
      [lastInvoiceID] = await db.query(`SELECT ID ,InvoiceNo FROM billmaster WHERE ID IN (SELECT MAX(ID) AS MaxID FROM billmaster WHERE CompanyID = '${CompanyID}' and ShopID = ${ShopID} and BillType = 1 )`);
    } else {
      [lastInvoiceID] = await db.query(`SELECT ID ,InvoiceNo FROM billmaster WHERE ID IN (SELECT MAX(ID) AS MaxID FROM billmaster WHERE CompanyID = '${CompanyID}' and BillType = 1 )`);
    }
  ...
  generateInvoiceNoForService2: async (CompanyID, ShopID, billDetailData, billMaseterData) => {
    ...
    if (billShopWiseBoolean) {
      [lastInvoiceID] = await db.query(`SELECT ID ,InvoiceNo FROM billmaster WHERE ID IN (SELECT MAX(ID) AS MaxID FROM billmaster WHERE CompanyID = '${CompanyID}' and ShopID = ${ShopID} and BillType = 0  )`);
    } else {
      [lastInvoiceID] = await db.query(`SELECT ID ,InvoiceNo FROM billmaster WHERE ID IN (SELECT MAX(ID) AS MaxID FROM billmaster WHERE CompanyID = '${CompanyID}' and BillType = 0  )`);
    }
  ...
  generateBillSno: async (CompanyID, ShopID) => {
    ...
    const [sNo] = await db.query(`select ID from billmaster where CompanyID = ${CompanyID} and ShopID = ${ShopID}`)
  ...
  generatePreOrderProduct: async (CompanyID, ShopID, Item, LoggedOnUser) => {
    ...
    const [supplierData] = await db.query(`select ID, Name, Status from supplier where CompanyID = ${CompanyID} and Name = 'PreOrder Supplier'`)
    console.log(supplierData, '===============supplierData');
    const [purchaseMasterData] = await db.query(`select ID,InvoiceNo,Quantity,SubTotal,DiscountAmount,GSTAmount,TotalAmount from purchasemasternew where CompanyID = ${CompanyID} and ShopID = ${ShopID} and purchasemasternew.SupplierID = ${supplierData[0].ID} order by purchasemasternew.ID desc`)
  ...
  update_c_report_setting: async (CompanyID, ShopID, CurrentDate) => {
    try {
      ...
      const [fetch_company_wise] = await db.query(`select OpeningStock, AmtOpeningStock  from creport where Date = '${date}' and CompanyID = ${CompanyID} and ShopID = 0`)
      ...
      const [fetch_shop_wise] = await db.query(`select OpeningStock, ClosingStock, AmtOpeningStock, AmtClosingStock from creport where Date = '${date}' and CompanyID = ${CompanyID} and ShopID = ${ShopID}`)
  ...
  getTotalAmountByBarcode: async (CompanyID, Barcode) => {
    try {
      ...
      const [fetchPurchaseDetail] = await db.query(`select UnitPrice, DiscountPercentage, GSTPercentage from purchasedetailnew where Status = 1 and CompanyID = ${CompanyID} and BaseBarCode = '${Barcode}'`);
  ...
  amt_update_c_report: async (CompanyID, ShopID, AddPurchase, AddPreOrderPurchase, DeletePurchase, AddSale, DeleteSale, AddPreOrderSale, DeletePreOrderSale, AddManualSale, DeleteManualSale, OtherDeleteStock, InitiateTransfer, CancelTransfer, AcceptTransfer, CurrentDate) => {
    try {
      ...
      const [update_shop_wise] = await db.query(`update creport set AmtAddPurchase=${shop_wise.addpurchase}, AmtAddPreOrderPurchase=${shop_wise.addpreorderpurchase}, AmtDeletePurchase=${shop_wise.deletepurchase}, AmtAddSale=${shop_wise.addsale}, AmtDeleteSale=${shop_wise.deletesale}, AmtAddPreOrderSale=${shop_wise.addpreordersale}, AmtDeletePreOrderSale=${shop_wise.deletepreordersale}, AmtAddManualSale=${shop_wise.addmanualsale}, AmtDeleteManualSale=${shop_wise.deletemanualsale}, AmtOtherDeleteStock=${shop_wise.otherdeletestock}, AmtInitiateTransfer=${shop_wise.initiatetransfer}, AmtCancelTransfer=${shop_wise.cancelTransfer}, AmtAcceptTransfer=${shop_wise.accepttransfer},AmtClosingStock=${shop_wise.closingstock} where ID = ${fetch_shop_wise[0].ID}`)
  ...
  update_pettycash_report: async (CompanyID, ShopID, Type, Amount, RegisterType, CurrentDate) => {
    try {
      ...
      const [fetch] = await db.query(`select * from pettycashreport where  CompanyID = ${CompanyID} and ShopID = ${ShopID} and RegisterType = '${RegisterType}' `)
      ...
      const [fetchPettyCash] = await db.query(`select * from pettycashreport where Date = '${date}' and CompanyID = ${CompanyID} and ShopID = ${ShopID} and RegisterType = '${RegisterType}' `)

      if (!fetchPettyCash.length) {

        const [fetchPettyCashBackDate] = await db.query(`select * from pettycashreport where CompanyID = ${CompanyID} and ShopID = ${ShopID} and RegisterType = '${RegisterType}' Order By ID desc`)
  ...
  update_pettycash_counter_report: async (CompanyID, ShopID, Type, Amount, CurrentDate) => {
    try {
      ...
      const processRegisterType = async (RegisterType) => {
        ...
        const [WithdrawalBalance] = await db.query(
        ...
        const [fetchPettyCash] = await db.query(
    ...
    try {
      ...
      if (CreditType === 'credit') {
        ...
        if (datum.Amount > 0) {
          ...
          const saveData = await db.query(`insert into rewardmaster(CompanyID, ShopID, CustomerID, InvoiceNo, PaidAmount,RewardPercentage,Amount, CreditType, Status, CreatedBy, CreatedOn) values(${CompanyID}, ${ShopID}, ${CustomerID}, '${InvoiceNo}', ${PaidAmount},${datum.RewardPercentage},${datum.Amount}, '${CreditType}', 1, ${LoggedOnUser}, now())`);
        }
      ...
      if (CreditType === 'customer_return_debit') {
        ...
        if (datum.Amount > 0) {
          console.log("reward_master datum ====> ", datum);
          const saveData = await db.query(`insert into rewardmaster(CompanyID, ShopID, CustomerID, InvoiceNo, PaidAmount,RewardPercentage,Amount, CreditType, Status, CreatedBy, CreatedOn) values(${CompanyID}, ${ShopID}, ${CustomerID}, '${InvoiceNo}', ${PaidAmount},${datum.RewardPercentage},${datum.Amount}, 'debit', 1, ${LoggedOnUser}, now())`);
        }
  ...
  getCustomerRewardBalance: async (CustomerID, CompanyID) => {
    try {
      ...
      const [CreditBalance] = await db.query(`select SUM(rewardmaster.Amount) as Amount from rewardmaster where Status = 1 and CompanyID = ${CompanyID} and CustomerID = ${CustomerID} and CreditType='credit'`)

      const [DebitBalance] = await db.query(`select SUM(rewardmaster.Amount) as Amount from rewardmaster where Status = 1 and CompanyID = ${CompanyID} and CustomerID = ${CustomerID} and CreditType='debit'`)
  ...
  getProductCountByBarcodeNumber: async (Barcode, CompanyID, ShopID) => {
    try {
      ...
      const [fetch] = await db.query(qry)
  ...
  getLocatedProductCountByBarcodeNumber: async (Barcode, CompanyID, ShopID) => {
    try {
      ...
      const [fetch] = await db.query(`select SUM(locationmaster.Qty) as LocatedQty from locationmaster where locationmaster.CompanyID = ${CompanyID} and locationmaster.ShopID = ${ShopID} and locationmaster.Barcode = '${Barcode}' and locationmaster.Status = 1`);
    ...
    try {
      ...
      if (Location.length) {
        for (let item of Location) {
          ...
          const [fetch] = await db.query(`select ID, Qty from locationmaster where Status = 1 and CompanyID = ${CompanyID} and ShopID = ${ShopID} and ProductTypeID = ${ProductTypeID} and ProductTypeName = '${ProductTypeName}' and Barcode = '${Barcode}' and ID = ${item.LocationMasterID} and LocationID = ${item.LocationID}`);
          ...
          if (fetch.length) {
            ...
            const [update] = await db.query(`update locationmaster set Qty = ${datum.Qty} where Status = 1 and ID = ${item.LocationMasterID} and LocationID = ${item.LocationID} and Barcode = '${Barcode}' and CompanyID = ${CompanyID} and ShopID = ${ShopID} `);
