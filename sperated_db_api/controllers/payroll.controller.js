...
module.exports = {
    save: async (req, res, next) => {
        ...
        try {
            ...
            const [doesExist] = await connection.query(`select ID from payroll where Status = 1 and EmployeeID=${Body.EmployeeID} and Month='${Body.Month}' and Year='${Body.Year}' and CompanyID=${CompanyID}`)
            ...
            const [paymentDetail] = await connection.query(`insert into paymentdetail(PaymentMasterID,BillID,BillMasterID,CustomerID,CompanyID,Amount,DueAmount,PaymentType,Credit,Status,CreatedBy,CreatedOn) values (${paymentMaster.insertId},'${datum.InvoiceNo}',${saveData.insertId},${datum.EmployeeID},${CompanyID},${datum.Salary},0,'Employee','Debit',1,${LoggedOnUser}, now())`)
            ...
            const [data] = await connection.query(`select * from payroll where CompanyID = ${CompanyID} and ShopID = ${shopid} and Status = 1 order by ID desc`)
            response.data = data
    ...
    delete: async (req, res, next) => {
        ...
        try {
            ...
            const [payment] = await connection.query(`select ID, PaymentMasterID from paymentdetail where Status = 1 and BillID='${doesExist[0].InvoiceNo}' and CompanyID = ${CompanyID} and PaymentType = 'Employee'`)
    ...
    update: async (req, res, next) => {
        ...
        try {
            ...
            const [update] = await connection.query(`update payroll set EmployeeID=${datum.EmployeeID}, ShopID = ${shopid} ,Month='${datum.Month}', Year='${datum.Year}',LeaveDays=${datum.LeaveDays},Salary=${datum.Salary},PaymentMode='${datum.PaymentMode}',CashType='${datum.CashType}',Comments='${datum.Comments}', UpdatedBy=${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID} and CompanyID = ${CompanyID}`)

            const [payment] = await connection.query(`select ID, PaymentMasterID from paymentdetail where Status = 1 and BillID='${doesExist[0].InvoiceNo}' and CompanyID = ${CompanyID} and PaymentType = 'Employee'`)


            const [updatePaymentMaster] = await connection.query(`update paymentmaster set ShopID = ${shopid}, PaymentMode='${datum.PaymentMode}',PayableAmount=${datum.Salary},PaidAmount=${datum.Salary},Comments='${datum.Comments}', UpdatedBy=${LoggedOnUser}, UpdatedOn=now() where CustomerID=${Body.ID} and PaymentType = 'Employee'  and ID =${payment[0].PaymentMasterID}`)

            const [updatePaymentDetail] = await connection.query(`update paymentdetail set Amount=${datum.Salary}, UpdatedBy=${LoggedOnUser}, UpdatedOn=now() where BillMasterID =${Body.ID} and PaymentType = 'Employee' and CompanyID = ${CompanyID} and BillID = '${doesExist[0].InvoiceNo}'`)

            console.log("Payroll Updated SuccessFUlly !!!");


            const [doesExistPettyCash] = await connection.query(`select ID, Amount, CashType, ShopID from pettycash where CompanyID = ${CompanyID} and InvoiceNo = '${doesExist[0].InvoiceNo}' and RefID = ${Body.ID} and Status = 1`)
            ...
            if (doesExistPettyCash.length && datum.PaymentMode.toUpperCase() === "CASH") {
                ...
                const [updatePettycash] = await connection.query(`update pettycash set ShopID=${shopid}, EmployeeID=${datum.EmployeeID}, CashType='${datum.CashType}',Amount='${datum.Salary}',Comments='${datum.Comments}', UpdatedBy=${LoggedOnUser},ShopID=${shopid}, UpdatedOn=now() where RefID = ${Body.ID} and CompanyID = ${CompanyID} and InvoiceNo = '${doesExist[0].InvoiceNo}'`)
