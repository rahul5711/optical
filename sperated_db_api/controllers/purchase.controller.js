...
module.exports = {
    ...
    create: async (req, res, next) => {
        try {
            ...
            const [doesExistInvoiceNo] = await db.query(`select ID from purchasemasternew where Status = 1 and InvoiceNo = '${PurchaseMaster.InvoiceNo}' and SupplierID = '${PurchaseMaster.SupplierID}' and CompanyID = ${CompanyID} and ShopID = ${shopid}`)
          ...
          //  console.log("purchaseDetail ===========>", purchaseDetail);
            ...
            let [detailDataForBarCode] = await db.query(`select * from purchasedetailnew where Status = 1 and PurchaseID = ${savePurchase.insertId} and CompanyID = ${CompanyID}`)
    ...
    update: async (req, res, next) => {
        try {
            ...
            const [doesExistInvoiceNo] = await db.query(`select ID from purchasemasternew where Status = 1 and InvoiceNo = '${PurchaseMaster.InvoiceNo}' and SupplierID = '${PurchaseMaster.SupplierID}' and CompanyID = ${CompanyID} and ShopID = ${shopid} and ID != ${PurchaseMaster.ID}`)
          ...
          //  console.log("doesExistSystemID =======>", doesExistSystemID);
            ...
            const [doesCheckPayment] = await db.query(`select ID, PaymentMasterID from paymentdetail where CompanyID = ${CompanyID} and PaymentType = 'Vendor' and BillMasterID = ${PurchaseMaster.ID}`)
            ...
            if (shouldUpdatePayment) {
                const [updatePaymentMaster] = await db.query(`update paymentmaster set PayableAmount = ${PurchaseMaster.TotalAmount} , PaidAmount = 0, UpdatedBy = ${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' where ID = ${doesCheckPayment[0].PaymentMasterID} and CompanyID = ${CompanyID}`)

                const [updatePaymentDetail] = await db.query(`update paymentdetail set BillID = '${PurchaseMaster.InvoiceNo}', Amount = 0 , DueAmount = ${PurchaseMaster.TotalAmount}, UpdatedBy = ${LoggedOnUser}, UpdatedOn='${req.headers.currenttime}' where ID = ${doesCheckPayment[0].ID} and CompanyID = ${CompanyID}`)
    ...
    getPurchaseById: async (req, res, next) => {
        try {
            ...
            const [PurchaseMaster] = await db.query(`select * from purchasemasternew  where Status = 1 and ID = ${ID} and CompanyID = ${CompanyID} `)

            const [PurchaseDetail] = await db.query(`select * from purchasedetailnew where  PurchaseID = ${ID} and CompanyID = ${CompanyID}  order by purchasedetailnew.ID desc`)

            const [Charge] = await db.query(`select * from purchasecharge where PurchaseID = ${ID} and CompanyID = ${CompanyID}`)
    ...
    delete: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select ID, SystemID from purchasemasternew where Status = 1 and CompanyID = '${CompanyID}' and ID = '${Body.ID}'`)
            ...
            const [doesExistProduct] = await db.query(`select ID from purchasedetailnew where Status = 1 and CompanyID = '${CompanyID}' and PurchaseID = '${Body.ID}'`)
            ...
            const [deletePurchase] = await db.query(`update purchasemasternew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID} and CompanyID = ${CompanyID}`)
    ...
    deleteProduct: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select ID, SystemID, Quantity,PurchaseID, TotalAmount from purchasedetailnew where Status = 1 and CompanyID = '${CompanyID}' and ID = '${Body.ID}'`)
            ...
            const [doesExistProductQty] = await db.query(`select ID from barcodemasternew where Status = 1 and CompanyID = '${CompanyID}' and PurchaseDetailID = '${Body.ID}' and CurrentStatus = 'Available'`)
            ...
            const [doesCheckPayment] = await db.query(`select ID, PaymentMasterID from paymentdetail where CompanyID = ${CompanyID} and BillID = '${Body.PurchaseMaster.InvoiceNo}' and BillMasterID = ${Body.PurchaseMaster.ID}`)
            ...
            const [deletePurchasedetail] = await db.query(`update purchasedetailnew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID} and CompanyID = ${CompanyID}`)

            console.log("Product Delete SuccessFUlly !!!");

            const [deleteBarcodeMasterNew] = await db.query(`update barcodemasternew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where PurchaseDetailID = ${Body.ID} and CompanyID = ${CompanyID} and CurrentStatus = 'Available'`)
            ...
            const [updatePaymentMaster] = await db.query(`update paymentmaster set PayableAmount = ${Body.PurchaseMaster.TotalAmount} , PaidAmount = 0, UpdatedBy = ${LoggedOnUser}, UpdatedOn=now() where ID = ${doesCheckPayment[0].PaymentMasterID} and CompanyID = ${CompanyID}`)

            const [updatePaymentDetail] = await db.query(`update paymentdetail set Amount = 0 , DueAmount = ${Body.PurchaseMaster.TotalAmount}, UpdatedBy = ${LoggedOnUser}, UpdatedOn=now() where ID = ${doesCheckPayment[0].ID} and CompanyID = ${CompanyID}`)

            const [fetchPurchaseMaster] = await db.query(`select * from purchasemasternew  where Status = 1 and ID = ${Body.PurchaseMaster.ID} and CompanyID = ${CompanyID} and ShopID = ${shopid}`)

            const gst_detail = await gstDetail(CompanyID, Body.PurchaseMaster.ID) || []

            fetchPurchaseMaster[0].gst_detail = gst_detail

            const [PurchaseDetail] = await db.query(`select * from purchasedetailnew where  PurchaseID = ${doesExist[0].PurchaseID} and CompanyID = ${CompanyID}`)
    ...
    deleteCharge: async (req, res, next) => {
        try {
            ...
            const [doesCheckPayment] = await db.query(`select * from paymentdetail where CompanyID = ${CompanyID} and BillID = '${Body.PurchaseMaster.InvoiceNo}' and BillMasterID = ${Body.PurchaseMaster.ID}`)
            ...
            const [deleteCharge] = await db.query(`update purchasecharge set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID} and CompanyID = ${CompanyID}`)

            console.log("Charge Delete SuccessFUlly !!!");

            const [Charge] = await db.query(`select * from purchasecharge where PurchaseID = ${doesExist[0].PurchaseID} and CompanyID = ${CompanyID}`)
            response.result.Charge = Charge;
            response.result.PurchaseMaster = fetchPurchaseMaster;
            response.message = "data delete sucessfully"
            return res.send(response);
    ...
    purchaseDetailPDF: async (req, res, next) => {
        try {
            ...
            const [supplier] = await db.query(`select * from supplier where CompanyID = ${CompanyID} and ID = ${PurchaseMasters.SupplierID}`)
    ...
    PrintBarcode: async (req, res, next) => {
        try {
            ...
            const [barcodeFormate] = await db.query(`select * from barcodesetting where CompanyID = ${CompanyID}`)
            const [barcode] = await db.query(`select * from barcodemasternew where CompanyID = ${CompanyID} and PurchaseDetailID = ${printdata[0].ID}`)
    ...
    barCodeListBySearchString: async (req, res, next) => {
        try {
            ...
            console.log(qry);
            let [purchaseData] = await db.query(qry);
    ...
    transferProduct: async (req, res, next) => {
        try {
            ...
            let [xferData] = await db.query(qry);
            let xferID = xferData.insertId;

            let [selectedRows] = await db.query(`
            ...
            let [xferList] = await db.query(qry1);
    ...
    acceptTransfer: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from transfermaster where ID = ${ID} and AcceptanceCode  = '${AcceptanceCode}' and CompanyID = ${CompanyID} and RefID = 0 and TransferStatus = 'Transfer Initiated'`)
    ...
    cancelTransfer: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from transfermaster where ID = ${ID} and CompanyID = ${CompanyID} and TransferStatus = 'Transfer Initiated'`)
    ...
    bulkTransferProductPDF: async (req, res, next) => {
        try {
            ...
            const [TransferFromShop] = await db.query(`select * from shop where ID = ${printdata.xMaster.TransferFromShop}`)
            const [TransferToShop] = await db.query(`select * from shop where ID = ${printdata.xMaster.TransferToShop}`)
    ...
    bulkTransferProductByID: async (req, res, next) => {
        try {
            ...
            let [master] = await db.query(`select * from transfer where CompanyID = ${CompanyID} and ID = ${ID}`);
            if (!master.length) {
                return res.send({ message: "Invalid Query Data" })
            }
    ...
    bulkTransferProductCancel: async (req, res, next) => {
        try {
            ...
            let [master] = await db.query(`select * from transfer where CompanyID = ${CompanyID} and ID = ${xMaster.ID}`);
            if (!master.length) {
                return res.send({ message: "Invalid Query Data" })
            }
            ...
            let [updateTransfer] = await db.query(`update transfer set Quantity = ${xMaster.Quantity}, TransferStatus = '${masterStatus}', UpdatedBy = ${LoggedOnUser}, UpdatedOn = now() where CompanyID = ${CompanyID} and ID = ${xMaster.ID}`)
    ...
    bulkTransferProductUpdate: async (req, res, next) => {
        try {
            ...
            let [updateTransfer] = await db.query(`update transfer set Quantity = ${xMaster.Quantity}, UpdatedBy = ${LoggedOnUser}, UpdatedOn = now() where CompanyID = ${CompanyID} and ID = ${xMaster.ID}`)
    ...
    bulkTransferProductAccept: async (req, res, next) => {
        try {
            ...
            let [master] = await db.query(`select * from transfer where CompanyID = ${CompanyID} and AcceptanceCode  = '${xMaster.AcceptanceCode}' and TransferStatus = 'Transfer Initiated' and ID = ${xMaster.ID}`);
            if (!master.length) {
                return res.send({ message: "Invalid Query Data" })
            }
            if (master[0].TransferStatus !== 'Transfer Initiated') {
            ...
            let [updateTransfer] = await db.query(`update transfer set TransferStatus = '${TransferStatus}', UpdatedBy = ${LoggedOnUser}, UpdatedOn = now() where CompanyID = ${CompanyID} and ID = ${xMaster.ID}`)
    ...
    barcodeDataByBarcodeNo: async (req, res, next) => {
        try {
            ...
            let [barcodelist] = await db.query(qry);
    ...
    updateBarcode: async (req, res, next) => {
        try {
            ...
            const [doesCheck] = await db.query(`select * from barcodemasternew where ID = ${ID}`)
            ...
            let [barcode] = await db.query(qry);
    ...
    getPurchasereportsExport: async (req, res, next) => {
        try {
            ...
            if (data.length) {
                for (let item of data) {
                    let [fetchGstType] = await db.query(`select * from purchasedetailnew where CompanyID = ${CompanyID} and PurchaseID = ${item.ID} LIMIT 1`)
    ...
    setProductExpiryDate: async (req, res, next) => {
        try {
            ...
            const [data] = await db.query(`select * from purchasedetailnew where CompanyID = ${CompanyID}`)
            ...
            if (data) {
                for (const item of data) {
                    ...
                    if (isValidDate(pName[pName.length - 1])) {
                        const [update] = await db.query(`update purchasedetailnew set ProductExpDate = '${pName[pName.length - 1]}', UpdatedBy = ${item.CreatedBy}, UpdatedOn = now() where ID = ${item.ID}`)
                    }
                }
            }
    ...
    createPreOrder: async (req, res, next) => {
        try {
            ...
            const [doesExistInvoiceNo] = await db.query(`select ID from purchasemasternew where Status = 1 and InvoiceNo = '${PurchaseMaster.InvoiceNo}' and CompanyID = ${CompanyID} and ShopID = ${shopid}`)
    ...
    deletePreOrderDummy: async (req, res, next) => {
        try {
            ...
            const [deletePurchasedetail] = await db.query(`update purchasedetailnew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${PurchaseDetail.ID} and CompanyID = ${CompanyID}`)

            console.log("Product Delete SuccessFUlly !!!");

            const [deleteBarcode] = await db.query(`update barcodemasternew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where purchaseDetailID = ${PurchaseDetail.ID} and CompanyID = ${CompanyID}`)
    ...
    deleteAllPreOrderDummy: async (req, res, next) => {
        try {
            ...
            for (const item of PurchaseDetail) {
                ...
                const [deletePurchasedetail] = await db.query(`update purchasedetailnew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${item.ID} and CompanyID = ${CompanyID}`)

                console.log("Product Delete SuccessFUlly !!!");

                const [deleteBarcode] = await db.query(`update barcodemasternew set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where purchaseDetailID = ${item.ID} and CompanyID = ${CompanyID}`)
    ...
    updatePreOrderDummy: async (req, res, next) => {
        try {
            ...
            const [updateBarcodeMaster] = await db.query(`update barcodemasternew set RetailPrice=${PurchaseDetail.RetailPrice}, WholeSalePrice=${PurchaseDetail.WholeSalePrice} where CompanyID = ${CompanyID} and PurchaseDetailID = ${PurchaseDetail.ID}`)
    ...
    getPurchaseByIdPreOrder: async (req, res, next) => {
        try {
            ...
            const [PurchaseMaster] = await db.query(`select * from purchasemasternew  where Status = 1 and ID = ${ID} and CompanyID = ${CompanyID} and ShopID = ${shopid} and PStatus = 1`)

            const [PurchaseDetail] = await db.query(`select 0 as Sel, purchasedetailnew.* from purchasedetailnew where  PurchaseID = ${ID} and CompanyID = ${CompanyID}`)
    ...
    deletePreOrder: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from purchasemasternew where Status = 1 and CompanyID = '${CompanyID}' and ID = '${Body.ID}'`)
            ...
            const [doesExistProduct] = await db.query(`select * from purchasedetailnew where Status = 1 and CompanyID = '${CompanyID}' and PurchaseID = '${Body.ID}'`)
    ...
    deleteProductPreOrder: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from purchasedetailnew where Status = 1 and CompanyID = '${CompanyID}' and ID = '${Body.ID}'`)
    ...
    updatePreOrder: async (req, res, next) => {
        try {
            ...
            const [doesExistInvoiceNo] = await db.query(`select ID from purchasemasternew where Status = 1 and InvoiceNo = '${PurchaseMaster.InvoiceNo}' and CompanyID = ${CompanyID} and ShopID = ${shopid} and ID != ${PurchaseMaster.ID}`)
    ...
    savePurchaseReturn: async (req, res, next) => {
        try {
            ...
            const [doesExistSystemCn] = await db.query(`select * from purchasereturn  where Status = 1 and SystemCn = '${PurchaseMaster.SystemCn}' and CompanyID = ${CompanyID} and ShopID = ${shopid}`)
            ...
            for (const item of purchaseDetail) {
                ...
                const [savePurchaseDetail] = await db.query(`insert into purchasereturndetail(ReturnID,CompanyID,PurchaseDetailID,ProductName,ProductTypeID,ProductTypeName,UnitPrice, Quantity,SubTotal,DiscountPercentage,DiscountAmount,GSTPercentage, GSTAmount,GSTType,TotalAmount,Barcode,Status,CreatedBy,CreatedOn,Remark)values(${savePurchaseReturn.insertId},${CompanyID},${item.PurchaseDetailID},'${item.ProductName}',${item.ProductTypeID},'${item.ProductTypeName}', ${item.UnitPrice},${item.Quantity},${item.SubTotal},${item.DiscountPercentage},${item.DiscountAmount},${item.GSTPercentage},${item.GSTAmount},'${item.GSTType}',${item.TotalAmount},'${item.Barcode}',1, ${LoggedOnUser},now(),'${item.Remark}')`)


                let count = 0;
                count = item.Quantity;

                let [fetch] = await db.query(`select barcodemasternew.ID from barcodemasternew left join purchasedetailnew on purchasedetailnew.ID = barcodemasternew.PurchaseDetailID where barcodemasternew.Status = 1 and barcodemasternew.Barcode = '${item.Barcode}' and purchasedetailnew.ProductName = '${item.ProductName}' and barcodemasternew.CurrentStatus = 'Available' and barcodemasternew.CompanyID = ${CompanyID} and barcodemasternew.ShopID = ${shopid} limit ${count}`)

                for (const ele of fetch) {
                    let [updateBarcode] = await db.query(`update barcodemasternew set CurrentStatus = 'Return To Supplier', BillDetailID = ${savePurchaseDetail.insertId} where Status = 1 and Barcode = '${item.Barcode}' and CurrentStatus = 'Available' and CompanyID = ${CompanyID} and ShopID = ${shopid} and ID = ${ele.ID}`)
                }
    ...
    updatePurchaseReturn: async (req, res, next) => {
        try {
            ...
            const [doesExistSystemCn] = await db.query(`select * from purchasereturn where Status = 1 and SystemCn = '${PurchaseMaster.SystemCn}' and CompanyID = ${CompanyID} and ShopID = ${shopid} and ID != ${PurchaseMaster.ID}`)

            if (doesExistSystemCn.length) {
                return res.send({ message: `Purchase Already exist from this SystemCn ${PurchaseMaster.SystemCn}` })
            }

            const [doesExistSupplierCn] = await db.query(`select * from purchasereturn where Status = 1 and SystemCn = '${PurchaseMaster.SystemCn}' and CompanyID = ${CompanyID} and ShopID = ${shopid} and ID = ${PurchaseMaster.ID}`)
            ...
            for (const item of purchaseDetail) {
                ...
                if (item.ID === null) {
                    ...
                    const [savePurchaseDetail] = await db.query(`insert into purchasereturndetail(ReturnID,CompanyID,PurchaseDetailID,ProductName,ProductTypeID,ProductTypeName,UnitPrice, Quantity,SubTotal,DiscountPercentage,DiscountAmount,GSTPercentage, GSTAmount,GSTType,TotalAmount,Barcode,Status,CreatedBy,CreatedOn,Remark)values(${purchase.ID},${CompanyID},${item.PurchaseDetailID},'${item.ProductName}',${item.ProductTypeID},'${item.ProductTypeName}', ${item.UnitPrice},${item.Quantity},${item.SubTotal},${item.DiscountPercentage},${item.DiscountAmount},${item.GSTPercentage},${item.GSTAmount},'${item.GSTType}',${item.TotalAmount},'${item.Barcode}',1,${LoggedOnUser},now(),'${item.Remark}')`)
    ...
    getPurchaseReturnById: async (req, res, next) => {
        try {
            ...
            const [PurchaseMaster] = await db.query(`select * from purchasereturn  where Status = 1 and ID = ${ID} and CompanyID = ${CompanyID} and ShopID = ${shopid}`)

            const [PurchaseDetail2] = await db.query(`select purchasereturndetail.*, purchasemasternew.InvoiceNo from purchasereturndetail left join purchasedetailnew on purchasedetailnew.ID = purchasereturndetail.PurchaseDetailID left join purchasemasternew on purchasemasternew.ID = purchasedetailnew.PurchaseID  where  purchasereturndetail.ReturnID = ${ID} and purchasereturndetail.CompanyID = ${CompanyID}`)

            const [PurchaseDetail] = await db.query(`select * from purchasereturndetail where  Status = 1 and ReturnID = ${ID} and CompanyID = ${CompanyID}`)
    ...
    deleteProductPR: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from purchasereturndetail where Status = 1 and CompanyID = '${CompanyID}' and ID = '${Body.ID}'`)
            ...
            const [doesExistSystemCn] = await db.query(`select * from purchasereturn where Status = 1 and SystemCn = '${Body.PurchaseMaster.SystemCn}' and CompanyID = ${CompanyID} and ShopID = ${shopid} and ID = ${Body.PurchaseMaster.ID}`)
            ...
            const [deletePurchasedetail] = await db.query(`update purchasereturndetail set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID} and CompanyID = ${CompanyID}`)

            console.log("Product Delete SuccessFUlly !!!");

            // update purchasemaster
            const [updatePurchaseMaster] = await db.query(`update purchasereturn set Quantity = ${Body.PurchaseMaster.Quantity}, SubTotal = ${Body.PurchaseMaster.SubTotal}, DiscountAmount = ${Body.PurchaseMaster.DiscountAmount}, GSTAmount=${Body.PurchaseMaster.GSTAmount}, TotalAmount = ${Body.PurchaseMaster.TotalAmount} , UpdatedBy = ${LoggedOnUser}, UpdatedOn=now() where CompanyID = ${CompanyID} and SystemCn = '${Body.PurchaseMaster.SystemCn}' and ShopID = ${Body.PurchaseMaster.ShopID}`)

            // update barcode

            const [updateBarcode] = await db.query(`update barcodemasternew set CurrentStatus = 'Available' , BillDetailID = 0  where BillDetailID = ${Body.ID}`)


            const [fetchPurchaseMaster] = await db.query(`select * from purchasereturn  where Status = 1 and ID = ${Body.PurchaseMaster.ID} and CompanyID = ${CompanyID} and ShopID = ${Body.PurchaseMaster.ShopID}`)

            const [PurchaseDetail2] = await db.query(`select * from purchasereturndetail where ReturnID = ${doesExist[0].ReturnID} and CompanyID = ${CompanyID}`)

            const [PurchaseDetail] = await db.query(`select * from purchasereturndetail where Status = 1 and ReturnID = ${doesExist[0].ReturnID} and CompanyID = ${CompanyID}`)
    ...
    deletePR: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from purchasereturn where Status = 1 and CompanyID = '${CompanyID}' and ID = '${Body.ID}'`)
            ...
            const [doesExistProduct] = await db.query(`select * from purchasereturndetail where Status = 1 and CompanyID = '${CompanyID}' and ReturnID = '${Body.ID}'`)
            ...
            const [deletePurchase] = await db.query(`update purchasereturn set Status=0, UpdatedBy= ${LoggedOnUser}, UpdatedOn=now() where ID = ${Body.ID} and CompanyID = ${CompanyID}`)
    ...
    supplierCnPR: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from purchasereturn where Status = 1 and CompanyID = '${CompanyID}' and ID = '${ID}'`)

            if (!doesExist.length) {
                return res.send({ message: "purchasereturn doesnot exist from this id " })
            }

            const [doesCheckCn] = await db.query(`select * from paymentdetail where CompanyID = ${CompanyID} and BillID = '${SupplierCn.trim()}' and PaymentType = 'Vendor Credit' and Credit = 'Credit'`)
            ...
            let [update] = await db.query(`update purchasereturn set SupplierCn = '${SupplierCn}', PurchaseDate = '${PurchaseDate}', CreatedOn=now(), UpdatedBy=${LoggedOnUser} where ID =${ID}`)
            ...
            const [savePaymentDetail] = await db.query(`insert into paymentdetail(PaymentMasterID,BillID,BillMasterID,CustomerID,CompanyID,Amount,DueAmount,PaymentType,Credit,Status,CreatedBy,CreatedOn)values(${savePaymentMaster.insertId},'${SupplierCn}',${ID},${supplierId},${CompanyID},${doesExist[0].TotalAmount},0,'Vendor Credit','Credit',1,${LoggedOnUser}, now())`)
    ...
    setbarcodemaster: async (req, res, next) => {
        try {
            ...
            let [detailDataForBarCode] = await db.query(`SELECT * FROM purchasedetailnew WHERE CompanyID = ${CompanyID} AND BaseBarCode IN (
                '10000351',
                '10000352',
                '10000353',
                '10000354',
    ...
    getInvoicePayment: async (req, res, next) => {
        try {
            ...
            if (PaymentType === 'Supplier') {
                ...
                const [due] = await db.query(`select SUM(purchasemasternew.DueAmount) as due from purchasemasternew where CompanyID = ${CompanyID} and SupplierID = ${PayeeName} and PStatus = 0 and Status = 1 and ID = ${PurchaseID}`)
    ...
    getCountInventoryReport: async (req, res, next) => {
        try {
            ...
            let [datum] = await db.query(`SELECT SUM(AddPurchase) AS TotalAddPurchase,
    ...
    getAmountInventoryReport: async (req, res, next) => {
        try {
            ...
            let [datum] = await db.query(`SELECT SUM(AmtAddPurchase) AS TotalAddPurchase,
    ...
    updateProductPrice: async (req, res, next) => {
        try {
            ...
            for (let item of ProductData) {
                ...
                let [fetchBarcode] = await db.query(`select * from barcodemasternew where CompanyID = ${CompanyID} and PurchaseDetailID = ${item.PurchaseDetailID} and ShopID = ${item.ShopID} and Status = 1 and CurrentStatus = 'Available' and Barcode = '${item.Barcode}'`)

                if (fetchBarcode.length !== item.Count) {
                    return res.send({ message: `Invalid Query Count/Available Barcode :- ${item.Barcode}` })
                }

                const [update] = await db.query(`update barcodemasternew set RetailPrice = ${item.RetailPrice}, WholeSalePrice = ${item.WholeSalePrice}, UpdatedOn=now() where CompanyID = ${CompanyID} and PurchaseDetailID = ${item.PurchaseDetailID} and Status = 1 and CurrentStatus = 'Available' and Barcode = '${item.Barcode}' `)
    ...
    getVendorDuePayment: async (req, res, next) => {
        try {
            ...
            let [datum] = await db.query(`SELECT SUM(purchasemasternew.Quantity) AS totalQty, SUM(purchasemasternew.GSTAmount) AS totalGstAmount, SUM(purchasemasternew.TotalAmount) AS totalAmount, SUM(purchasemasternew.DiscountAmount) AS totalDiscount, SUM(purchasemasternew.SubTotal) AS totalSubTotal, SUM(purchasemasternew.DueAmount) AS totalDueAmount FROM purchasemasternew LEFT JOIN supplier ON supplier.ID = purchasemasternew.SupplierID WHERE purchasemasternew.Status = 1 AND supplier.Name != 'PreOrder Supplier' AND purchasemasternew.CompanyID = ${CompanyID}  ${Parem}`)
    ...
    savePhysicalStockProduct: async (req, res, next) => {
        try {
            ...
            const [saveMaster] = await db.query(`INSERT INTO physicalstockcheckmaster(CompanyID, ShopID, InvoiceNo, InvoiceDate, TotalAvailableQty, TotalPhysicalQty, TotalQtyDiff, Remark, Status, CreatedBy,CreatedOn) values(${CompanyID}, ${shopid}, '${xMaster.InvoiceNo}', '${xMaster.InvoiceDate}', ${xMaster.TotalAvailableQty},${xMaster.TotalPhysicalQty}, ${xMaster.TotalQtyDiff}, '${xMaster.Remark}', 1,${LoggedOnUser}, now())`);

            // insertId

            for (let item of xDetail) {
                const [saveDetail] = await db.query(`INSERT INTO physicalstockcheckdetail(MasterID,CompanyID,ShopID,ProductTypeID,ProductTypeName,ProductName,Barcode,RetailPrice,WholeSalePrice,AvailableQty,PhysicalAvailableQty,QtyDiff, Status,CreatedBy,CreatedOn) values(${saveMaster.insertId},${CompanyID},${shopid},${item.ProductTypeID},'${item.ProductTypeName}','${item.ProductName}','${item.Barcode}',${item.RetailPrice},${item.WholeSalePrice},${item.AvailableQty},${item.PhysicalAvailableQty},${item.QtyDiff}, 1,${LoggedOnUser}, now())`)
            }
    ...
    getPhysicalStockProductByID: async (req, res, next) => {
        try {
            ...
            const [fetchMaster] = await db.query(`select * from physicalstockcheckmaster  where Status = 1 and ID = ${ID} and CompanyID = ${CompanyID} `)

            const [fetchDetail] = await db.query(`select *, AvailableQty AS Available,PhysicalAvailableQty AS PhysicalAvailable  from physicalstockcheckdetail where MasterID = ${ID} and CompanyID = ${CompanyID}  order by physicalstockcheckdetail.ID desc`)
    ...
    updatePhysicalStockProduct: async (req, res, next) => {
        try {
            ...
            const [doesExist] = await db.query(`select * from physicalstockcheckmaster where CompanyID = ${CompanyID} and ShopID = ${shopid} and Status = 1 and ID = ${xMaster.ID}`);

            if (!doesExist.length) {
                return res.send({ message: "Invalid ID Data" })
            }

            const [updateMaster] = await db.query(`Update physicalstockcheckmaster set InvoiceNo='${xMaster.InvoiceNo}', InvoiceDate='${xMaster.InvoiceDate}', TotalAvailableQty=${xMaster.TotalAvailableQty}, TotalPhysicalQty=${xMaster.TotalPhysicalQty}, TotalQtyDiff=${xMaster.TotalQtyDiff}, Remark='${xMaster.Remark}', UpdatedBy=${LoggedOnUser}, UpdatedOn=now() where CompanyID = ${CompanyID} and ShopID = ${shopid} and ID = ${xMaster.ID}`);

            // insertId

            for (let item of xDetail) {
                const [updateDetail] = await db.query(`update physicalstockcheckdetail set AvailableQty=${item.AvailableQty},PhysicalAvailableQty=${item.PhysicalAvailableQty},QtyDiff=${item.QtyDiff} where ID = ${item.ID} and CompanyID = ${CompanyID}`)
            }
    ...
    getLocationStockProductList: async (req, res, next) => {
        try {
            ...
            if (data.length) {
                for (let item of data) {
                    const [fetch] = await db.query(`select SUM(Qty) as Located from locationmaster where CompanyID = ${CompanyID} and ShopID = ${shopid} and Barcode = '${item.Barcode}' and Status = 1`);
    ...
    saveProductLocation: async (req, res, next) => {
        try {
            ...
            let [data] = await db.query(`INSERT INTO locationmaster(CompanyID, ShopID, ProductTypeID, ProductTypeName, ProductName, Barcode, LocationID, Qty, Status, CreatedBy, CreatedOn, UpdatedBy, UpdatedOn)values(${CompanyID}, ${shopid}, ${ProductTypeID}, '${ProductTypeName}','${ProductName}','${Barcode}',${LocationID}, ${Qty}, 1, ${LoggedOnUser}, now(), ${LoggedOnUser}, now())`);
    ...
    updateProductLocation: async (req, res, next) => {
        try {
            ...
            let [data] = await db.query(`UPDATE locationmaster set LocationID = ${LocationID}, Qty = ${Qty}, UpdatedBy= ${LoggedOnUser}, UpdatedOn = now() where ID = ${ID} and CompanyID = ${CompanyID} and ShopID = ${shopid}`);
    ...
    deleteProductLocation: async (req, res, next) => {
        try {
            ...
            let [data] = await db.query(`UPDATE locationmaster set Status = 0, UpdatedBy= ${LoggedOnUser}, UpdatedOn = now() where ID = ${ID} and CompanyID = ${CompanyID} and ShopID = ${shopid}`);
